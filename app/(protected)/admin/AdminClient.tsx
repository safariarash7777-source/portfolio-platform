...
import {
  Users, PieChart, Mail, Search, PlusCircle,
  Trash2, Save, CheckCircle2, AlertCircle, ChevronDown,
} from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        ...
        <div className="grid grid-cols-3 gap-3 md:flex md:gap-3">
          {[
            ...
            { l: "ارزیابیشده", v: users.filter((u) => u.risk_category).length },
            { l: "پرتفوی دارند", v: users.filter((u) => u.has_portfolio).length },
          ].map((s) => (
            <div key={s.l} className="card px-4 py-3 text-center" style={{ minWidth: 90 }}>
              <div className="font-display text-xl font-bold" style={{ color: "var(--navy-deep)" }}>{s.v}</div>
  ...
  return (
    <div className="card-elevated p-6">
      <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
        <h3 className="font-display font-bold text-lg" style={{ color: "var(--navy-deep)" }}>لیست کاربران</h3>
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
          style={{ background: "var(--surface-2)", border: "1px solid var(--line)" }}>
          <Search size={14} style={{ color: "var(--text-3)" }} />
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="جستجوی نام ایمیل یا کد ملی..."
            className="bg-transparent outline-none text-sm w-52" style={{ color: "var(--text)" }} />
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ color: "var(--text-3)" }}>
              {["نام و نام خانوادگی", "کد ملی", "موبایل", "ایمیل", "پروفایل ریسک", "پرتفوی"].map((h) => (
                <th key={h} className="text-right py-3 px-3 text-xs font-bold border-b"
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="py-10 text-center text-sm" style={{ color: "var(--text-3)" }}>کاربری یافت نشد.</td></tr>
            ) : (
              filtered.map((u) => (
                <tr key={u.id} className="border-b hover:bg-[var(--surface-2)] transition-colors"
                  ...
                  <td className="py-3 px-3">
                    {u.risk_category ? (
                      ...
                      <span className="text-xs" style={{ color: "var(--text-3)" }}>ارزیابی نشده</span>
                    )}
                  </td>
                  <td className="py-3 px-3">
                    {u.has_portfolio ? (
...
function PortfolioTab({ users }: { users: UserRow[] }) {
  ...
  const [allocations, setAllocations] = useState<Allocation[]>([{ ...EMPTY_ALLOC }]);
  ...
  const addRow = () => { if (allocations.length < 8) setAllocations((a) => [...a, { ...EMPTY_ALLOC }]); };
  ...
  const handleSave = async () => {
    ...
    if (allocations.some((a) => !a.asset.trim())) { setError("نام دارایی برای همه ردیفها الزامی است"); return; }
    ...
    try {
      ...
      const { error: dbErr } = await supabase.from("portfolios").upsert({
        user_id: selectedUserId, allocations, notes: notes.trim() || null,
    ...
    <div className="card-elevated p-6 space-y-6">
      ...
      <div className="space-y-1.5">
        <label className="block text-sm font-bold" style={{ color: "var(--text-2)" }}>
          انتخاب کاربر<span className="mr-1" style={{ color: "var(--danger)" }}>*</span>
        </label>
        <div className="relative">
          <select className="input appearance-none" value={selectedUserId}
            onChange={(e) => setSelectedUserId(e.target.value)} disabled={saving}
        ...
        {selectedUser?.has_portfolio && (
          <p className="text-xs" style={{ color: "var(--warning)" }}>
            این کاربر از قبل پرتفوی دارد. ذخیره اطلاعات قبلی را بازنویسی میکند.
          </p>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-sm font-bold" style={{ color: "var(--text-2)" }}>تخصیص داراییها</label>
        ...
        <div className="grid text-xs font-bold py-2 px-3 rounded-lg"
          style={{ color: "var(--text-3)", background: "var(--surface-2)", gridTemplateColumns: "1fr 90px 1fr 40px", gap: "0.75rem" }}>
          <span>نام دارایی</span><span>درصد (%)</span><span>یادداشت (اختیاری)</span><span />
        </div>
        {allocations.map((row, i) => (
          <div key={i} className="grid items-center gap-3" style={{ gridTemplateColumns: "1fr 90px 1fr 40px" }}>
            <input type="text" className="input" placeholder="مثال: صندوق درآمد ثابت"
              value={row.asset} onChange={(e) => updateRow(i, "asset", e.target.value)} disabled={saving} />
            <input type="number" className="input text-center" placeholder="۰" min={0} max={100}
              value={row.pct || ""} onChange={(e) => updateRow(i, "pct", Number(e.target.value))}
              disabled={saving} dir="ltr" />
            <input type="text" className="input" placeholder="یادداشت..."
              value={row.note} onChange={(e) => updateRow(i, "note", e.target.value)} disabled={saving} />
            <button type="button" onClick={() => removeRow(i)}
              disabled={allocations.length <= 1 || saving}
              className="flex items-center justify-center rounded-lg transition-colors"
              style={{
                width: 36, height: 36, background: "rgba(185,28,28,0.07)",
                color: "var(--danger)", border: "1px solid rgba(185,28,28,0.2)",
        ...
        {allocations.length < 8 && (
          <button type="button" onClick={addRow} disabled={saving}
            className="btn btn-outline w-full" style={{ fontSize: "0.8rem", padding: "0.5rem" }}>
            <PlusCircle size={14} />افزودن دارایی
          </button>
        )}
      </div>

      <div className="space-y-1.5">
        <label className="block text-sm font-bold" style={{ color: "var(--text-2)" }}>یادداشت مشاور (اختیاری)</label>
        <textarea className="input resize-none" rows={3} placeholder="توضیحات تکمیلی برای کاربر..."
      ...
      {success && (
        <div className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm"
          ...
          <CheckCircle2 size={15} />پرتفوی با موفقیت ذخیره شد.
        </div>
      )}

      <button type="button" onClick={handleSave} className="btn btn-gold" disabled={saving}>
        <Save size={16} />{saving ? "در حال ذخیره..." : "ذخیره پرتفوی"}
      </button>
    </div>
  );
}
...
function WaitlistTab({ waitlist }: { waitlist: WaitlistRow[] }) {
  ...
  return (
    <div className="card-elevated p-6">
      <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
        <h3 className="font-display font-bold text-lg" style={{ color: "var(--navy-deep)" }}>لیست انتظار</h3>
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
          style={{ background: "var(--surface-2)", border: "1px solid var(--line)" }}>
          <Search size={14} style={{ color: "var(--text-3)" }} />
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="جستجوی ایمیل..." className="bg-transparent outline-none text-sm w-44"
            style={{ color: "var(--text)" }} dir="ltr" />
        </div>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={2} className="py-10 text-center text-sm" style={{ color: "var(--text-3)" }}>
                {waitlist.length === 0 ? "هنوز کسی ثبتنام نکرده است." : "نتیجهای یافت نشد."}
              </td></tr>
            ) : (
              filtered.map((r) => (
                <tr key={r.id} className="border-b hover:bg-[var(--surface-2)] transition-colors"
                  style={{ borderColor: "var(--line)" }}>
                  <td className="py-3 px-3" dir="ltr" style={{ color: "var(--text)" }}>{r.email}</td>
      ...
      <p className="text-xs mt-4" style={{ color: "var(--text-3)" }}>مجموع {waitlist.length} ثبتنام در لیست انتظار</p>
    </div>
  );
}
