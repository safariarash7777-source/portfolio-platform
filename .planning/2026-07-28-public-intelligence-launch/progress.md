...
## `P2-G2-006` — تمرینِ stagingِ مسیرِ لید (۲۰۲۶-۰۷-۳۰)

مجوز: `AUTHORIZE_GATE2_STAGING: YES` · `PRODUCTION_AUTHORIZATION: NO`.
نتیجه: **`PARTIALLY_PROVEN`**.

### تصحیحِ یک فرضِ ورودیِ مأموریت

متنِ مأموریت `lqfcyihuthdoqybwptxh` را «Production یا حساسِ نامعلوم» فرض کرده بود.
شواهدِ خودِ ریپو خلافش را می‌گوید و این مهم است، چون اگر بر اساسِ آن فرض عمل
می‌شد ممکن بود پروژهٔ واقعیِ Production با staging اشتباه گرفته شود:

- `DD-011` (DECIDED) و `SD-002`: `lqfcyihuthdoqybwptxh` **منسوخ و غیرِ Production** است.
- `uooeygybrniptzdxuzhj` **پروژهٔ فعالِ Production** است — چهار فایلِ `sql/phase1*.sql`
  صریحاً می‌گویند روی همین اجرا شده‌اند.

پس تنها پروژهٔ موجود در حساب، **خودِ Production** بود و هیچ stagingی وجود نداشت.
یک پروژهٔ **رایگان** (`get_cost` = ماهانه ۰) ساخته شد: `oqjcvkzyvhqnphopedpn`.
(برنچِ Supabase ساخته **نشد** — ساعتی ۰٫۰۱۳۴۴ دلار است و منبعِ پولی بدونِ تأییدِ
تازه ممنوع بود.)

### آنچه اثبات شد

migration روی staging سالم اجرا شد؛ نیمهٔ مینی‌اپ و احرازِ هویتِ وبهوک سرتاسر کار
کردند؛ هیچ دادهٔ شخصی و هیچ سکرتی در لاگ‌ها نبود؛ تمرینِ شکست (سکرتِ غلط → ۴۰۱،
لیدِ محلی سالم، بازیابی پس از اصلاح) کامل انجام شد.

### آنچه اثبات نشد

**هاپِ آخر — نوشتنِ ردیف در `public.leads` از مسیرِ اپلیکیشن.** دو مانعِ محیطی:
`B-029` (کلیدِ service-roleِ staging قابلِ دریافت نیست) و `B-030` (شبکهٔ محیطِ اجرا
`*.supabase.co` را مسدود می‌کند). `B-001`/`B-002`/`B-003` **باز می‌مانند**.

### یافتهٔ امنیتی — اصلاح‌شده پیش از Production

`REVOKE ALL … FROM anon` کافی نبود: `authenticated` امتیازِ کاملِ Supabase را نگه
داشته بود، از جمله `TRUNCATE`. **RLS روی `TRUNCATE` اعمال نمی‌شود**، پس محرمانگی
برقرار بود ولی یکپارچگی نه. Security Advisor آن را نگرفت — گرنت‌ها باید جدا از RLS
سنجیده شوند. فایل اصلاح شد و `lib/leads/grants.test.ts` (۹ تست، شکست‌پذیریِ گارد
عملاً آزموده شد) جلوی برگشتش را می‌گیرد.

### دست‌نخورده‌ها

Production (هیچ SQL، هیچ اتصال، هیچ متغیر، هیچ استقرار) · PR #91 · `D-024` ·
PR #74 · PR #75 · پرداخت.
