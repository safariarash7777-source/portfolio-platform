...
const CASES = [
  ...
  },
  {
    ...
    file: "lib/intelligence/approval-queue.ts",
    from: '    href: "/admin/drafts",',
    to: '    href: "/admin/manage?tab=drafts",',
    test: "lib/intelligence/approval-queue.test.ts",
  },
  // ── بازبینیِ نامزدهای موتور (FOLLOWUP-001) ────────────────────────────
  {
    name: "مجوز روی اقدامِ نوشتنی",
    defect: "کنارگذاشتن بدونِ بررسیِ نقش انجام شود — هر کاربرِ واردشده صف را ببندد",
    file: "lib/drafts/service.ts",
    from: "  const gate = await authorize(gateway);\n  if (!gate.ok) return { status: gate.status, body: { error: DENIED } };\n\n  const id = input.id.trim();",
    to: "  const gate = { ok: true, userId: \"x\" };\n\n  const id = input.id.trim();",
    test: "lib/drafts/service.test.ts",
  },
  {
    name: "صفِ نامزدها fail-closed می‌ماند",
    defect: "پرس‌وجوی مردود «۰ مورد» گزارش شود — صفِ خوانده‌نشده شبیهِ صفِ خالی",
    file: "lib/drafts/contracts.ts",
    from: "      count: null,\n      state: \"unavailable\",",
    to: "      count: 0,\n      state: \"empty\",",
    test: "lib/drafts/contracts.test.ts",
  },
  {
    name: "فقط نامزدِ بازنشده نمایش داده می‌شود",
    defect: "فیلترِ وضعیت برداشته شود — موردِ بسته‌شده دوباره در صف ظاهر شود",
    file: "lib/drafts/contracts.ts",
    from: "  const pending = rows.filter((r) => r.status === PENDING);",
    to: "  const pending = rows.slice();",
    test: "lib/drafts/contracts.test.ts",
  },
  {
    name: "RLS گیتِ دوم می‌ماند",
    defect: "مسیرِ نامزدها service-role بگیرد و سیاست‌های RLS را دور بزند",
    file: "app/api/admin/drafts/route.ts",
    from: 'import { createClient } from "@/lib/supabase/server";',
    to: 'import { createClient } from "@/lib/supabase/server";\nimport { createAdminClient } from "@/lib/supabase/admin";',
    test: "lib/drafts/service.test.ts",
  },
  {
    name: "نامزد «تأییدشده» نمی‌شود",
    defect: "ماژول وضعیتِ `approved` بنویسد — یک تأییدِ معلق بدونِ هیچ انتشاری",
    file: "lib/drafts/service.ts",
    from: 'export const DRAFT_STATUS = { reads: PENDING, writes: DISMISSED } as const;',
    to: 'export const DRAFT_STATUS = { reads: PENDING, writes: "approved" } as const;',
    test: "lib/drafts/service.test.ts",
  },
  {
    name: "فعلِ مسیر سفیدفهرست است",
    defect: "سفیدفهرستِ فعل برداشته شود — بدنهٔ دلخواه به لایهٔ اقدام برسد",
    file: "app/api/admin/drafts/route.ts",
    from: 'if (String(body.action ?? "") !== "dismiss") {',
    to: 'if (false) {',
    test: "lib/drafts/service.test.ts",
  },
];
