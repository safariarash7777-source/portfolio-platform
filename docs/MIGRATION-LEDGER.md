...
| `phase21_cron_runs.sql` | **NOT_APPLIED** | دفترِ اجرای cron (`P2-G2-012`). **طراحی، بدونِ اجرا** — نه Production، نه Staging. چرا لازم است: «آخرین اجرای موفق» تا امروز از دیتابیس **قابل دانستن نبود** — `alerts` وقتی هشداری نباشد هیچ ردیفی نمی‌نویسد، و `telegram-sync` فقط با پستِ تازه درج می‌کند، پس «ردیفِ تازه نیست» با «اجرا نشد» یکسان به‌نظر می‌رسید. قابلِ اجرابودن روی **Postgresِ یک‌بارمصرفِ محلی** سنجیده شد و ۲۸ تستِ رفتاری روی دو پروفایلِ امتیاز سبز است (RLS، گرنت‌ها، گذارها، قیدها)؛ آن بررسی **اجرا روی محیطِ واقعی نیست** |
| `phase20_intelligence_model.sql` | **NOT_APPLIED** | مدلِ بازطراحی‌شدهٔ هوشمندی بازار (`G3-001`، ADR-005): **۱۵ جدول تازه** شامل چندشاهدی، اتصال FK به `signals`، provenance انسانی/ایجنت و سبد مرجع نسخه‌دار. هیچ شیٔ موجودی تغییر نمی‌کند. migration واقعی در CI روی Postgres 16 یک‌بارمصرف و دو پروفایل امتیاز اجرا می‌شود؛ انتشار ناقص، شناسهٔ سیگنال نامعتبر، مجموع وزن غیر ۱۰۰٪، mutation پس از انتشار/نهایی‌سازی و DELETE/TRUNCATE رد می‌شوند. این فقط rehearsal محلی/CI است؛ نه Staging و نه Production. پیش‌نیاز اجرا: بسته‌شدن ریسک‌های Gate 2 (`DD-024`) و rehearsal مستقل migration. |
