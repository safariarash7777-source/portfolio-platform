...
| `phase24_payment_entitlement.sql` | **NOT_APPLIED** (نه Production، نه Staging) | مسیرِ «پرداخت → دسترسی» (`P2-PAY-001`). **افزایشی**: یک ایندکسِ یکتای جزئی روی `entitlements(user_id, source)` و یک تابعِ `finalize_paid_access`. هیچ جدول/ستون/سیاست/دادهٔ موجودی را تغییر نمی‌دهد و `create_payment`/`verify_payment`/`fail_payment` دست‌نخورده‌اند — تابعِ تازه `verify_payment` را **صدا می‌زند**، جایگزینش نمی‌کند. ⚠️ **ترتیب اجباری**: پس از `phase23` و **پیش از** استقرارِ کد؛ اگر کد زودتر برود هر پرداخت به `access_pending` می‌افتد. قابلِ اجرا بودن و رفتارش روی **Postgresِ یک‌بارمصرفِ CI** با ۲۵ تستِ رفتاری سنجیده شد (idempotency، هم‌زمانیِ واقعیِ چهار تراکنشِ موازی، بازگشتِ تراکنش هنگام شکستِ اعطا، بازگشت هنگامِ نوشتنِ بی‌صدا بی‌اثر، بایندِ مبلغ/کاربر/ثبت‌نام، و `permission denied` واقعی برای `authenticated`)؛ هر گارد با بازگرداندنِ نقصِ اصلی آزموده شد که قرمز می‌شود. **این اجرا روی محیطِ واقعی نیست** — رویه در `docs/RUNBOOK-payment-activation.md` |
