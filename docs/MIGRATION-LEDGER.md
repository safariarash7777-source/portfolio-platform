...
| `phase34_research_workbook_versions.sql` | **NOT_APPLIED** · پشتِ دروازهٔ بکاپ | ذخیرهٔ سروریِ کاربرگِ پژوهش (#142): دو جدولِ **append-only** (`research_workbook_versions`، `research_workbook_reviews`)، فقط ادمین (RLS + `profiles.role`)، `anon` هیچ، `authenticated=SELECT,INSERT`، `service_role=SELECT`؛ `UPDATE/DELETE/TRUNCATE` برای هیچ نقشی — و برای مالک هم تریگر رد می‌کند. هر ذخیره نسخهٔ تازه است (`UNIQUE(workbook_id,version)` + گاردِ «بدونِ پرش»)؛ تأییدِ داخلی فقط روی آخرین نسخه و فقط اگر کفِ شاهد (`research_workbook_has_evidence`) برقرار باشد؛ نویسنده و زمان از نشست، نه از بدنه. **وابستگی:** فقط `auth.users`/`profiles` — به phase20/22 (که روی Production نیستند) وابسته نیست. **شاهد:** روی Postgres 16 یک‌بارمصرف، ۲۰ تست روی دو پروفایلِ امتیاز (`lib/intelligence/workbook-store.integration.test.ts`)؛ همین تست یک نقصِ واقعی گرفت (بدنهٔ بی‌کلیدِ `evidence` از کفِ شاهد رد می‌شد — `NULL <> 'array'`) که پیش از commit اصلاح شد. اجرای دوباره no-op است. **بازگشت:** `DROP` دو جدول و چهار تابع — تا وقتی ردیفی ثبت نشده بی‌خطر. تا اجرا، API ۵۰۳ صادق می‌دهد و صفحه به مسیرِ فایل برمی‌گردد. Production دست‌نخورده. |
