...
| `phase20_intelligence_model.sql` | **NOT_APPLIED** | مدلِ دادهٔ هوشمندیِ بازار (`G3-001`، ADR-005). **طراحی، بدونِ اجرا** — نه Production، نه Staging. ۹ جدولِ `intel_*`؛ هیچ شیٔ موجودی تغییر نمی‌کند و کارنامهٔ `signals`/`signal_outcomes` دست‌نخورده می‌ماند. قابلِ اجرابودنش روی یک **Postgresِ یک‌بارمصرفِ محلی** بررسی شد (۹ جدول، RLS روی همه، `authenticated` بدونِ TRUNCATE/DELETE، ادعای بی‌شاهد و انتشارِ بی‌تأییدکننده هر دو رد شدند)؛ آن بررسی **اجرا روی محیطِ واقعی نیست**. پیش‌نیازِ اجرا: بسته‌شدنِ ریسک‌های Gate 2 (`DD-024`) |
