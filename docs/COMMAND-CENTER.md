...
## تحویل Claude — ۱۴۰۵/۰۶/۲۸ · صحت دادهٔ بنیادی (#142) — PR #150

**محدودهٔ این نوبت:** فقط صحتِ انتخابِ گزارش در دادهٔ بنیادی.
**شاخهٔ مسئول:** `claude/codal-amendment-precedence`، منشعب از `main`.
**استقلال:** با #144 و #149 تداخلِ فایلی ندارد؛ به هر ترتیبی قابلِ ادغام است.

### دسترسیِ تازه‌ای که باز شد

خواندنِ پروژهٔ اصلی `uooeygybrniptzdxuzhj` از راهِ MCP فراهم شد — همان چیزی که
#142 را ماه‌ها مسدود کرده بود. فقط `SELECT`؛ هیچ نوشتن، هیچ migration، هیچ
دادهٔ خصوصی.

### باگ: اصلاحیه‌ها نامرئی بودند

انتخابِ گزارشِ هر دوره فقط `audited` و سپس `id` را می‌دید. `id` ترتیبِ **درج**
است نه انتشار، و `published_at` روی **هیچ‌کدام** از ۳٬۴۳۹ ردیفِ ن-۱۰ وجود
ندارد.

| سنجه | مقدار |
|---|---|
| دوره‌های دارای هم اصلاحیه هم نسخهٔ اولیه | ۲۸۰ |
| نسخهٔ اولیه فقط به‌خاطر `id` برنده می‌شد | **۲۵۷** |
| از آن میان، ارقام واقعاً متفاوت | **۱۷** |

نمونهٔ واقعی — شپنا، ۳ماههٔ ۱۴۰۴/۰۳/۳۱: اصلاحیه `id=9064` با فروش
۱٬۲۲۸٬۳۴۳٬۵۶۳ در برابر نسخهٔ اولیه `id=9065` با ۱٬۲۲۴٬۴۹۲٬۱۸۷. قاعدهٔ قبلی
**رقمِ باطل‌شده** را برمی‌گرداند.

تقدمِ تازه: حسابرسی‌شده ← اصلاحیه (در همان وضعیتِ حسابرسی) ← `id` بزرگ‌تر.
سیگنال از ستونِ `title` خوانده می‌شود، پس اصلاح روی هر ۳٬۴۳۹ ردیفِ موجود
**بدونِ بک‌فیل و بدونِ دست‌زدن به سوابق** کار می‌کند.

### دو تصحیحِ فهمِ خودم که ثبت شد

۱. اول نتیجه گرفتم «هیچ عددی در جدول نیست»، چون `data->>'revenue'` روی همهٔ
   ردیف‌ها `null` بود. **غلط بود:** ارقام تودرتو زیر `data.standalone` هستند و
   نامِ سودِ خالص `net_profit` است نه `net_income`. در
   `SPEC-symbol-fundamental-card.md` ثبت شد تا نفرِ بعد دوباره نیفتد.
۲. نمونهٔ اولِ تستم شناسه‌های ساختگی داشت و **اتفاقی سبز می‌ماند**. شناسه‌های
   واقعی از دیتابیس گرفته شد تا تست شانسی نباشد.

### چه چیزی بررسی شد و **درست بود** (کارِ تازه لازم نداشت)

- **تفکیکِ ساعت‌ها در میزِ تحلیل:** `lib/desk/clock.ts` دو ساعتِ جدا با برچسبِ
  «دادهٔ منبع» و «خوانده‌شده» دارد و `clock.test.ts` متفاوت‌بودنشان را تثبیت
  می‌کند. `changeRadar.ts` هم صریح نوشته که `published_at` وجود ندارد و
  `captured_at` فقط «کِی ما دیدیمش» است.
- **رابطهٔ حسابیِ گزارش‌ها:** در فولاد/شپنا/وبملت،
  `gross_profit = revenue − cogs` در همهٔ ردیف‌ها دقیقاً برقرار و واحد یکدستِ
  «میلیون ریال» است.
- **ادعای «تجدید ارائه تأییدشده» دربارهٔ شپنا:** در اسناد نیست. چیزی برای
  اصلاح نبود و چیزی هم اضافه نشد.

### آنچه ادعا نمی‌شود

- **تطبیق با خودِ فایلِ رسمیِ کدال انجام نشده.** آنچه اثبات شد سازگاریِ
  درونیِ دادهٔ ذخیره‌شده است، نه برابری با سندِ اصلی. برای آن باید اکسلِ همان
  اطلاعیه دوباره خوانده و مقابله شود.
- سلامت و تأخیرِ دریافتِ داده هنوز جایی دیده نمی‌شود — کارِ جداست.
- هیچ ردیفی اصلاح یا بازنویسی نشد؛ فقط **انتخاب** عوض شد.

### شاهد آزمون

`test:calc` **۸۸** · `test:core` **۱۰۴۸** · `typecheck` · `lint:ci` · `build` ·
`scan:secrets` — همه سبز روی `b4dc67f`. ۹ تستِ تازه با شناسه و رقمِ واقعی؛ روی
نسخهٔ معیوب سه‌تایشان می‌شکنند (با برگرداندنِ موقتِ نقص اثبات شد).

- **نقطه ادامه:** بازبینی مستقل روی #150؛ سپس تطبیق با فایلِ رسمیِ کدال، و بعد سلامتِ دریافتِ داده.
