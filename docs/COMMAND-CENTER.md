...
| **B-029** | **کلیدِ service-role برای هیچ محیطِ staging قابلِ دریافت نیست** — نه از MCPِ Supabase (فقط کلیدهای publishable/anon می‌دهد)، نه از Management API (توکنِ دسترسی در محیط نیست)، نه از خودِ دیتابیس (`app.settings.jwt_secret` ست نیست، `vault.secrets` خالی) | 🟠 HIGH | Ops / Staging | ARASH (اپراتور) | 2026-07-30 | B-003، G2-006 | آرش یا اپراتور کلیدِ service-roleِ پروژهٔ staging را از داشبوردِ Supabase در محیطِ اجرا ست کند. بدونِ آن، هاپِ آخرِ لید (نوشتن در `public.leads`) روی هیچ محیطی اثبات‌پذیر نیست | تلاشِ واقعی در `G2-006`: `get_publishable_keys` فقط `anon` و `sb_publishable_…` برگرداند؛ `SUPABASE_ACCESS_TOKEN` در محیط نبود؛ پرس‌وجوی فقط‌خواندنی روی staging: `jwt_secret_available=false`, `vault_secret_count=0` | **OPEN** · VERIFIED |
| **B-030** | **سیاستِ شبکهٔ محیطِ اجرا میزبانِ `*.supabase.co` را مسدود می‌کند** — پس حتی با کلیدِ درست هم نمی‌توان از این sandbox به Supabase نوشت | 🟡 MEDIUM | Ops / Tooling | ENGINEERING | 2026-07-30 | B-029، G2-006 | تمرینِ هاپِ آخر باید از محیطی اجرا شود که خروجی‌اش به Supabase باز است (مثلاً Preview روی Vercel یا ماشینِ اپراتور) | لاگِ واقعیِ اپلیکیشن در `G2-006`: `Host not in allowlist: oqjcvkzyvhqnphopedpn.supabase.co` · `curl` هم `CONNECT tunnel failed, response 403` گرفت | **OPEN** · VERIFIED |
