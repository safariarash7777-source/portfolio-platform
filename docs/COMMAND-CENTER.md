...
| **B-053** | **خطِ استقرارِ رله شکسته است — اتصالِ مستقیمِ مخزن با `"port" is required` رد می‌شود** | 🔴 CRITICAL | Relay / استقرار | **ARASH** (کنسول + سکرت) | 2026-09-14 | بستهٔ A · #126 · #127 — هیچ‌کدام روی رله زنده نیستند | **۱)** اتصالِ مخزن را در کنسولِ `arsadata` قطع کن (وگرنه دو منتشرکننده). **۲)** API Token بساز و در GitHub با نامِ `LIARA_API_TOKEN` ثبت کن. **۳)** پورتِ اپ را صریحاً `3400` ذخیره کن. **۴)** Actions → **Deploy relay** → `sha=1d7e8f1`. جزئیات و پذیرش: [`RUNBOOK-relay-deploy.md`](./RUNBOOK-relay-deploy.md) | **✅ تصحیحِ دو ادعای قبلیِ من:** «هیچ استقراری موفق نبوده» و «نسخهٔ زنده شناسه ندارد» **هر دو غلط بودند**. کنسول: نسخهٔ فعال `v46` · کامیت `6570aaf` · شاخه `main` · `#58` · موفق و «استقرارِ فعلی» · platform `node` · پورتِ نسخه `3400`. نمونه‌گیریِ من از رکوردهای *شکستِ اتصالِ مخزن* بود، نه فهرستِ release‌ها. **آنچه اثبات شد:** (۱) خطا مالِ **اتصالِ مخزن** است نه Actions — مخزن فقط `ci.yml` دارد و هیچ‌جا لیارا را صدا نمی‌زند. (۲) **علت داخلِ مخزن نیست**: `relay/liara.json` در `6570aaf` (همان v46ِ موفق) و در `main@1d7e8f1` بایت‌به‌بایت یکی است، و `liara.json` ریشه‌ای **هرگز وجود نداشته**. پس متغیر سمتِ کنسول است و **هیچ ویرایشِ مخزنی اثبات‌شده درستش نمی‌کند**. (۳) نسخهٔ زنده واقعاً `6570aaf` است — دو شاهدِ مستقل: لاگِ `history push ok: 4 sections` (در `6570aaf` آرایه ثابت و چهارتایی است، در `main` از `IR_HISTORY_SECTIONS` می‌آید و پیش‌فرض ۲ است) و پرس‌وجوی ۱۰:۳۴Z که هر چهار بخش را در حالِ نوشتن نشان داد. ⚠️ «بیلد از ریشهٔ مخزن» همچنان **فرضیه** است (سازگارترین توضیح، چون `"port" is required` خطای اعتبارسنجیِ درخواست است نه شکستِ بیلد) — لاگ خوانده نشده. **رفع:** `.github/workflows/deploy-relay.yml` — `--app arsadata --path relay --port 3400`، فقط پس از CI سبز روی همان SHA، با فیلترِ مسیرِ `relay/` (چون «استقرارِ بدونِ اختلال» خاموش است) و پیش‌پروازی که اگر `BRSAPI_BUDGET_ENFORCE_LEGACY=1` بود انتشار را متوقف می‌کند — `phase28` اجرا نشده (`brsapi_budget_days` و `brsapi_budget_lease` هر دو `false`، ۱۰:۳۴Z)، پس آن پرچم با کدِ تازه دادهٔ بازار را می‌ایستاند | **OPEN** · VERIFIED |
