...
## وضعیت جاری — ۱۴۰۵/۰۷/۰۱ (۲۰۲۶-۰۹-۲۳) · `P2-DELIVERY-CLOSURE-001`

> مبنا: `main@ddaeb97` — از ۲۰۲۶-۰۹-۱۷ هیچ ادغامی نشده. SHA و CI هر بسته همان روز از GitHub خوانده شد، نه از گزارش قبلی. بخش‌های تاریخ‌دارِ پایین‌تر **سابقه‌اند**.
>
> پنج برچسبِ جدا: **آمادهٔ بازبینی** · **آمادهٔ ادغام** · **ادغام‌شده** · **منتشرشده** · **پذیرفته در مسیرِ واقعی**. هیچ‌کدام از دیگری استنتاج نمی‌شود.

### بسته‌ها

| PR | head | پایه | CI همان SHA | وضعیت | باز / مانع |
|---|---|---|---|---|---|
| **#151** زیرساخت: تریگرِ CI + symlink | کُدِ `682704c` + کامیت‌های سندیِ همین جدول | `main` | ۷/۷ ✅ | **آمادهٔ ادغام** | فقط تأییدِ ادغام. پیش‌نیازِ #153 و #155. شاهدِ تازهٔ symlink پایین‌تر |
| **#153** رله: برچسبِ جلسه + نوشتنِ idempotent (B-055) | `cae459f` | #151 | ۷/۷ ✅ | آمادهٔ بازبینی | پس از ادغام، استقرارِ لیارا جدا؛ روزِ گذار دیگر داده از دست نمی‌رود (`eod_displaced:<تاریخ>`) |
| **#155** بکاپ: `pgurl.sh` مشترک، رمز بیرون از argv، e2eِ خودِ ps1 (B-057) | `13e7ba5` | #151 | ۷/۷ ✅ (قرمزِ `a647661` رفع شد) | آمادهٔ بازبینی | PASSِ بکاپ فقط با اجرای واقعی روی لپ‌تاپ |
| **#150** تقدمِ اصلاحیه | `24beb20` | `main` | ۷/۷ ✅ | آمادهٔ بازبینی | **بدونِ #154 ناقص است** |
| **#154** یک انتخابِ گزارش برای کارت/فصل/TTM/P/E + دامنهٔ شرکت/تلفیقی | `e34211c` | #150 | ۷/۷ ✅ | آمادهٔ بازبینی | B-056 اثبات‌نشده، ولی رقمِ مبهم دیگر با تیکِ سبز به شرکت نسبت داده نمی‌شود |
| **#144** دارایی و مقایسه | `48cea22` | `main` | ۷/۷ ✅ (۰۹-۱۷) | آمادهٔ بازبینی | `phase32`؛ بازبینیِ قراردادِ هدف (تحویلِ Codex) |
| **#149** هشدار | `09d4893` | #144 | روی GitHub هنوز فقط ۲ چکِ پیش‌نمایش؛ **معادلِ کاملِ CI محلی سبز** (پایین) | آمادهٔ بازبینی (CIِ GitHub پس از #151) | اصلاحِ تریگر به‌صورتِ patchِ `7e6fd47` به مالک تحویل شد (sha256 با `40760451871a09ca` شروع می‌شود؛ روی `09d4893` تمیز اعمال می‌شود و `MIN_PASS=377` را نگه می‌دارد). push به این شاخه را محیط رد کرد و به شاخهٔ دیگری فرستاده نشد |
| **#143** لغوِ اعلامیه | `ae06f0a` | `main` | ۷/۷ ✅ (۰۹-۱۷) | آمادهٔ بازبینی | گیتِ #148 روی پایه‌اش هست |
| **#152** دسته‌بندیِ صندوق + سیستمِ بصری | `26a4cf1` | `main` | ۷/۷ ✅ · `mergeable=clean` | آمادهٔ بازبینی | جلسهٔ مالک بیکار است؛ پذیرشِ باقی‌مانده بدونِ push به شاخه‌اش انجام شد (پایین). **Preview با دادهٔ واقعی باقی است** |
| **#113** پرداخت ← دسترسی | `b64ec25` | `main@2344179` | ۷/۷ روی پایهٔ **کهنه** | جایگزین با #156 | `mergeable_state=dirty`؛ `HOLD_BY_OWNER` (B-025)؛ `D-024` |
| **#156** همان #113، هم‌تراز با `main` | `fbb627e` | `main` | ۷/۷ ✅ روی `fbb627e` با گیتِ بسته (`2fc7f50` سبز بود ولی گیت بی‌اثر — پایین) | آمادهٔ بازبینی · `HOLD_BY_OWNER` پابرجا | بدونِ تعارض؛ `MIN_PASS=417` از کلاسترِ نو |
| **#157** کاربرگِ پژوهش: ذخیره، بازکردن، نسخهٔ دوم، تأییدِ داخلی (#142) | `819cbf1` | `main` | ۷/۷ ✅ (`MIN_PASS=356`) | آمادهٔ بازبینی | `phase34` **اجرا نشده** (دروازهٔ بکاپ)؛ تا آن موقع API ۵۰۳ صادق و صفحه روی مسیرِ فایل |

**ترتیبِ ادغام:** #151 → (#153، #155) · #150 → #154 · #144 → #149. سه زنجیره مستقل‌اند. #156 و #157 هر دو `MIN_PASS` را عوض می‌کنند: دومی پس از ادغامِ اولی باید **دوباره اندازه‌گیری** شود، نه جمع.

**هیچ migrationی روی Production اجرا نشده.** هیچ بسته‌ای منتشر یا در مسیرِ واقعی پذیرفته نشده.

### یافته‌های این نوبت — هر کدام با شاهدِ Production (فقط‌خواندنی)

- **B-054 — اثباتِ زنده.** #153 و #155 (روی شاخهٔ #151) هر ۷ چک را گرفتند؛ #154 روی شاخهٔ #150 فقط ۲ چک گرفت تا همان اصلاح به آن آورده شد و سپس ۷/۷. نخستین PRهای روی‌هم‌چیده در این مخزن که واقعاً سنجیده شدند.
- **B-055 — پایانیِ هر جلسه زیرِ برچسبِ جلسهٔ بعد.** از ۱۴۰۵/۰۴/۲۷ هر ۵۰٬۲۱۱ ردیفِ `relay_eod` ساعتِ ۰۰ تهران نوشته شده‌اند. شاهدِ مستقل از کد: سهمِ نمادهای مثبتِ `market_breadth` با `relay_eod`ِ یک برچسب بعد ۵ واحد فاصله دارد، با همان برچسب ۲۱ واحد. اصلاح در #153. **ردیف‌های گذشته اصلاح نمی‌شوند** (append-only)؛ نمای تصحیح در خواندن تصمیمِ مالک است.
  - **هزینهٔ گذار — رفع شد، نه پذیرفته:** کلیدِ یکتای `(symbol, trade_date)` بدونِ `source` است و در روزِ استقرار برچسبِ آن جلسه از نیمه‌شب پر شده. #153 ردیف‌های جابه‌جاشده را در `ir_market_snapshots` با کلیدِ `eod_displaced:<تاریخ>` نگه می‌دارد، پس پایانیِ آن جلسه گم نمی‌شود. **زمانِ دقیقِ استقرار همچنان باید ثبت شود** — مرزِ هر نمای تصحیحِ بعدی.
  - خطرِ پنهان: همین کلید یعنی ردیفِ `brsapi_candle` برای همان نماد و روز، درجِ ۵۰۰تاییِ رله را با 409 می‌شکند. امروز فعال نیست (۴۵ ردیفِ candle از ۰۴/۲۹، ۲ نمادِ مشترک).
- **#150 به فصل‌ها نرسیده بود.** صفحهٔ نماد دو انتخابگر داشت: کارت از `dedupeByPeriod`، فصل/TTM/P/E از `lib/core/quarterly` با قاعدهٔ قدیمیِ `id`. و `id` در بک‌فیلِ آرشیو **برعکسِ** زمانِ انتشار است: فخاس سالانهٔ ۱۴۰۳ سودِ خالصِ ۷۳٬۳۹۸٬۰۵۱ نشان می‌داد، گزارشِ دیرتر ۶۱٬۵۳۶٬۵۹۰ (۱۶٪). `raw.date_publish/time_publish` روی هر ۳٬۵۵۳ ردیفِ ن-۱۰ هست. اصلاح در #154.
- **B-056 — `standalone`ِ گزارشِ تلفیقی احتمالاً رقمِ گروه است.** در ۱۹۳ از ۱۹۷ دوره‌ای که هر دو نوع دارند گزارشِ تلفیقی برنده می‌شود؛ ۵۵۰ دوره فقط تلفیقی دارند؛ ۱٬۱۴۳ ردیف. پارسر اولین جدولِ سود و زیان را برمی‌دارد. **اثبات نشده** — سندِ اصلیِ کدال لازم است.
- **B-057 — بکاپِ ویندوز.** پایپِ PowerShell رشتهٔ اتصال را با CRLF می‌فرستد و `read -r` فقط LF را برمی‌دارد ← `database "postgres\r" does not exist`. بازتولید با psql و Postgresِ واقعی. اصلاح در #155. همچنین: رمز در argvِ psqlِ داخلِ کانتینر دیده می‌شود (مستند شد، رفع نشد).

### نوبتِ دوم ۱۴۰۵/۰۷/۰۱ — شاهدهای تازه

- **گیتِ تعدادِ تستِ دیتابیس می‌توانست بی‌صدا رد شود.** `2fc7f50` (#156) با `MIN_PASS=__MEASURE__` push شد و CI سبز ماند: `[ n -lt x ]` با عددِ نامعتبر خطا می‌دهد و درونِ `if` خطا «نادرست» خوانده می‌شود. `fbb627e` عدد را ثبت کرد و گیت را «بسته» کرد (غیرعدد مردود؛ مقایسهٔ معکوس `! -ge`)؛ ۹ ترکیب با `bash -eo pipefail` شبیه‌سازی شد. **همین ضعف روی `main` هست** (امروز بی‌خطر چون عدد معتبر است) و با ادغامِ #156 بسته می‌شود.
- **symlinkِ `node_modules` در `main` مطلق است** (`→ /home/user/portfolio-platform/node_modules`). یک worktree از `main` در این محیط، `node_modules`ِ پوشهٔ اصلی را پاک کرد (۳۵۳ بسته → ۱ ورودی) و `npm ci` لازم شد. شاهدِ عینیِ فوریتِ #151.
- **#149 — معادلِ کاملِ CI روی `09d4893`، محلی** (Postgres 16 نو، بدونِ push): `typecheck` · `lint:ci` · `test:core` ۱۱۶۱/۰ · `test:calc` ۷۹/۰ · `test:relay` · `test:deploy` ۶۹/۰ · `build` · `test:db` **۳۷۷/۰/۰ اسکیپ، ۰ `not ok`** (= `MIN_PASS` خودِ شاخه) · اسکنِ سکرت. این **CIِ GitHub نیست**؛ آن پس از ادغامِ #151 می‌آید.
- **#152 — پذیرشِ چیدمان**، روی `26a4cf1` در worktreeِ جدا (بدونِ push): ۴ مسیر (`/`، `/market`، `/market/funds`، `/market/map`) × ۳۶۰/۳۹۰/۱۴۴۰ × روشن/تیره = ۲۴ نما، همه ۲۰۰، `dir=rtl`، اسکرولِ افقی ۰، خطای کنسول ۰، هدفِ لمسیِ زیرِ ۲۴px صفر. **حدود:** بدونِ داده (Supabase از اینجا ۰۰۰) — پنلِ تفکیک با داده فقط با fixtureِ جلسهٔ مالک در ۳۹۰/۱۴۴۰ دیده شده؛ حالتِ خالی صادق است («تا وقتی ردیف معتبر نرسد، عدد نشان نمی‌دهد»). **باقی‌مانده:** بازکردنِ Preview با دادهٔ واقعی در ۳۶۰ توسطِ آرش؛ `/admin` و `/terminal` خارج از ممیزی.
- **میزِ تحلیل (#142) → #157.** جدول‌های `intel_*` روی Production **وجود ندارند** (خواندنِ `pg_class`)، پس ذخیرهٔ دائمی به هر حال پشتِ migration است. #157 به آن‌ها وابسته نیست. تستِ دیتابیسش یک نقصِ واقعی گرفت پیش از commit: بدنهٔ بی‌کلیدِ `evidence` از کفِ شاهد رد می‌شد.

### ظرفیتِ دیتابیس — بازاندازه‌گیریِ ۱۴۰۵/۰۷/۰۱ و طرحِ رسیدن به زیرِ سقف

**مبنای طرح `docs/DATA-RETENTION-AND-ARCHIVE-PLAN.md` و `HOT-COLD-STORAGE-DESIGN.md` روی شاخهٔ PR #119 است** (ادغام‌نشده). اینجا فقط اعدادِ امروز و آنچه از آن زمان عوض شده ثبت می‌شود؛ سندِ موازی ساخته نشد.

- **سقف:** سازمان روی پلنِ **free** است (`get_organization`) ⇒ ۵۰۰ MB. امروز **۱٬۲۹۵٫۵ MB = ۲٫۶ برابرِ سقف**. `ACTIVE_HEALTHY` و موفق‌بودنِ نوشتن‌های رله یعنی اجرا **هنوز** اعمال نشده، نه اینکه مشکل حل شده.
- ❌ **تصحیحِ گزارشِ ۰۶/۳۱ همین سند:** «دو اقدام ≈ ۵۵۴ MB» به ۷۴۱ MB می‌رسید — همچنان بالای سقف. و «`raw` حذف‌پذیر است» غلط بود (پایین).

**تفکیکِ بی‌دوباره‌شماری** (heap = فایلِ اصلی، TOAST جدا، index جدا؛ سهمِ هر بخش از جمعِ `pg_column_size` ضربِ ضریبِ صفحهٔ ۱٫۱۰):

| بخش | heap | TOAST | index | کل |
|---|---|---|---|---|
| `ir_market_history` · `stocks`+`funds` | ~۸٫۵ | ~۴۵۹ | ~۰٫۸ | **~۴۶۸** |
| `ir_market_history` · `gold`+`currency` | ~۰٫۶ | ~۱۹ | ~۰٫۱ | ~۲۰ |
| `symbol_history` · کندل پیش از ۱۳۹۹ | ۲۰۱ | ۰ | ۱۱۰ | **۳۱۱** |
| `symbol_history` · `brsapi_history` پیش از ۱۳۹۹ | ۳۲ | ۰ | ۹ | ۴۱ |
| `symbol_history` · ۱۳۹۹ به بعد (همه منابع) | ۲۷۰ | ۰ | ۱۳۴ | ۴۰۴ |
| سایرِ جدول‌ها + سیستم | | | | ~۵۱ |

(index در `symbol_history` ۱۱۹ بایت در هر ردیف است، که ۴۵ بایتش مالِ indexِ تکراری است.)

**چه چیزی فقط در `raw` هست — خطِ قرمز:**

| منبع | کلیدها | چرا حذف‌شدنی نیست |
|---|---|---|
| `brsapi_candle` (۱٫۹۴M) | `open` `high` `low` `jdate` `time` `candle` | ستون‌های تایپ‌دارِ OHLC در ۱۰۰٪ این ردیف‌ها **تهی**‌اند؛ `jdate` تاریخِ جلالیِ **خودِ منبع** و سندِ `trade_date` است |
| `relay_eod` (۵۰k) | `nav_toman` `bubble_percent` `change_percent` حجم‌های حقیقی/حقوقی | تنها منبعِ تاریخچهٔ NAV (`lib/core/navHistory.ts`)؛ `change_percent` تنها راهِ بازسازیِ پایانیِ دیروزِ منبع بود (همان که B-055 را ثابت کرد) |
| `brsapi_history` (۱۲۴k) | `py` `tno` ارزش/حجم/تعدادِ گروه‌ها | پایانیِ دیروز و تعدادِ معاملات جای دیگری نیست |
| `codal_reports` | `date_publish` `time_publish` `link_excel` `announcement_link` | زمانِ انتشار فقط اینجاست (`published_at` تهی — B-045)؛ #154 به آن تکیه دارد |

**خواننده‌ها — فقط کدِ برنامه نیست.** در دیتابیس: صفر view، function، matview یا `cron.job`. در مخزن: رله (نوشتن + **هرسِ ۱۸۰روزه**)، اسکریپت‌های بکاپ (شمارش)، و برنامه. عمیق‌ترین نیاز: خروجیِ CSV کلِ تاریخچه؛ P/E و تخصیصِ ترمینال ۱٬۵۰۰ روزِ معاملاتی (≈۶٫۲ سال)؛ بقیه ≤۴۰۰ روز.

⚠️ **دادهٔ بی‌خواننده خودبه‌خود پاک می‌شود.** هرسِ ۱۸۰روزهٔ رله (`relay/server.mjs`) ردیف‌های `stocks`/`funds` را از **۱۴۰۵/۱۰/۱۹ تا ۱۴۰۵/۱۲/۲۳** روزانه حذف می‌کند — بدونِ آرشیو و بدونِ آزادکردنِ فضای فیزیکی. «خواننده ندارد» ≠ «بی‌ارزش است»: این تنها سابقهٔ لحظه‌ایِ ۳۰دقیقه‌ایِ کلِ بازار در ۶۵ روز است. **اگر نگه‌داشتنش ارزش دارد، بکاپِ معتبر باید پیش از ۱۴۰۵/۱۰/۱۹ گرفته شود.** تصمیمِ مالک.

**طرح — هیچ‌کدام اجرا نشده؛ هر گام بکاپِ معتبرِ تازه + مجوزِ جدای همان گام:**

| # | اقدام | پس از آن | روشِ بازپس‌گیریِ فیزیکی | قفل | فضای موقت / WAL | برگشت |
|---|---|---|---|---|---|---|
| ۱ | `stocks`/`funds` به فایلِ آرشیو (از بکاپِ معتبر)، سپس جدولِ تازه فقط با `gold`/`currency` و جابه‌جاییِ نام | **≈۸۲۷** | جابه‌جاییِ جدول: فقط ~۲۰ MB بازنویسی. `DELETE` به‌تنهایی **هیچ** فضایی برنمی‌گرداند (TOAST) | `ACCESS EXCLUSIVE` چند میلی‌ثانیه هنگامِ rename | ~۲۰ / ~۲۰ | فایلِ آرشیو |
| ۲ | `DROP INDEX CONCURRENTLY idx_symbol_history_symbol_date` — #119 با `EXPLAIN` نشان داد planner هرگز انتخابش نمی‌کند | **≈۷۳۲** | فوری | بدونِ قفلِ نوشتن | ~۰ | `CREATE INDEX CONCURRENTLY` |
| ۳ | ردیف‌های پیش از ۱۳۹۹ (≈۹۹۴k، همه منابع) به آرشیوِ سرد؛ CSV مسیرِ خواندن از آرشیو بگیرد | **≈۴۲۵** | جدولِ تازهٔ «گرم» + جابه‌جایی. `VACUUM FULL` کلِ جدول را برای چند دقیقه قفل می‌کند و صفحه‌های نماد خطا می‌دهند | کوتاه، **ولی** نوشتنِ رله (۱۴:۰۰–۰۰:۳۰) و بک‌فیلِ کندل در پنجرهٔ کپی متوقف یا بازپخش شود | ~۴۰۰ / ~۴۰۰ | آرشیو + جدولِ قدیم تا تأیید |
| ۴ | OHLC از `raw` به ستون‌های تایپ‌دار برای ردیف‌های گرمِ کندل؛ `jdate` به ستونِ متنیِ فشرده؛ بقیهٔ `raw`ِ کندل حذف — هم‌زمان با گامِ ۳ در همان بازنویسی | **≈۳۴۰** | همان بازنویسی | همان | همان | همان |

**ترتیب مهم است:** گام‌های ۱ و ۲ پیش از ۳ و ۴، چون بازنویسیِ `symbol_history` تا ~۴۰۰ MB فضای موقت و WAL می‌خواهد و روی پلنِ رایگان همین ~۵۶۰ MBِ آزادشده است که جایش را فراهم می‌کند. ارقامِ ستونِ «پس از آن» برآوردند (±۵٪) و پس از هر گام باید با `pg_database_size` دوباره خوانده شوند.

**حاشیه:** رشدِ امروز ~۴٫۷ MB در هفته (رله ۲٫۰، `codal_reports` ۱٫۷ و `codal_feed` ۰٫۶ با بک‌فیلِ جاری، کندل ۰٫۳؛ `gold`/`currency` زیرِ هرسِ ۱۸۰روزه ثابت ~۲۰ MB). با گام ۱–۴ (≈۳۴۰): تا آستانهٔ هشدارِ ۴۰۰ حدودِ **۱۲ هفته**، تا سقف حدودِ **۳۴ هفته**. با پایانِ بک‌فیلِ کدال رشد به ~۲٫۵ MB در هفته و حاشیه به ~۱٫۲ سال می‌رسد. **فقط گام ۱–۳ (≈۴۲۵) زیرِ سقف است ولی کم‌حاشیه** (~۱۶ هفته).

حسابِ گام‌ها: ۱ = −۴۶۸ · ۲ = −۹۵ · ۳ = heapِ ۲۳۳ + index با ۷۴ بایت در ردیف (پس از گامِ ۲) × ۹۹۴k = ۷۴ ⇒ −۳۰۷ · ۴ = `raw`ِ کندلِ گرم ۱۱۷٫۸ منهای ~۳۹ برای OHLC و `jdate`ِ تایپ‌دار، × ۱٫۱۰ ⇒ −۸۷.

**محلِ آرشیو** (Storageِ Supabase، R2 یا فایلِ محلیِ بکاپ) و **افقِ گرم** (۱۳۹۹ یا کوتاه‌تر) تصمیمِ مالک‌اند. خرید پلن در دامنهٔ این مأموریت نیست.

### حفاظتِ `main`

`protected: false` — push مستقیم، force-push و ادغامِ بدونِ CI همه ممکن‌اند؛ CI فعلاً فقط مشورتی است. (rulesetها با ابزارِ موجود قابلِ خواندن نبودند.)

**پیشنهادِ دقیق** (تغییرِ تنظیمات فقط با مجوزِ جدا):
- چکِ الزامی: **فقط `CI Gate`**. این job هر چهار job را `needs` دارد، `if: always()` است و هر نتیجه‌ای جز `success` — حتی skipped — را رد می‌کند؛ پس یک نامِ پایدار کافی است و با افزودنِ job تازه نمی‌شکند.
- PR پیش از ادغام الزامی؛ force-push و حذفِ شاخه ممنوع.
- تأییدِ بازبین الزامی **نشود** — مخزنِ تک‌مالک است و ادغامِ خودِ مالک را قفل می‌کند.
- «به‌روز بودن با پایه» الزامی **نشود** — زنجیره‌های روی‌هم‌چیده را بی‌دلیل قرمز می‌کند.

### PRهای قدیمی — superseded؟ (هیچ‌کدام بسته نشد)

| PR | جایگزین | وضعیت |
|---|---|---|
| #75، #91 | #113 — متنِ #113 صریحاً «Supersedes #75 and #91» | پس از تعیینِ تکلیفِ #113 بسته شوند، نه پیش از آن |
| #119 سهمیه | احتمالاً #121 (`1d7e8f1`) که رشدِ `ir_market_history` را مهار کرد | diff پیش از بستن مقایسه شود |
| #104 ممیزیِ UX، #117 طراحیِ عملیاتیِ هوشمندی | چیزی جایگزینشان ادغام نشده | مرجع بمانند؛ تصمیمِ مالک/Codex |
| #74 | پایه‌اش `develop` است، نه خطِ تحویل | تصمیمِ مالک |
| شاخهٔ `claude/backup-structural-verify` | اصلِ stdinِ آن در `main` هست | PR ندارد؛ مقایسهٔ باقی‌مانده لازم |

### مانعِ دقیقِ این محیط

- **push به شاخه‌های موجودِ دیگران/جلسه‌های قبل** توسطِ طبقه‌بندِ مجوزِ حالتِ خودکار رد شد (`Modify Shared Resources`). به همین دلیل #149 هنوز CI نگرفته. رفع: مجوز در تنظیماتِ Claude Code، یا ادغامِ #151 و #144 تا #149 تریگرِ درست را از پایه بگیرد.
- **`codal.ir`** از پراکسی ۰۰۰ — تطبیق با سندِ اصلی (موجِ دو) و اثباتِ B-056 ممکن نیست. تنها راهِ دیگر، پلِ رله، نوشتن در Production است.
- **`pwsh` نصب نیست و pull ایمیجِ Docker از پراکسی 403** — خودِ اسکریپتِ بکاپِ ویندوز اینجا اجرا نمی‌شود.
- **Vercel MCP** وصل نمی‌شود (`ERR_PROXY_TUNNEL 403`) — وضعیتِ استقرار و SHAی Production خوانده نشد.

### اقدامِ بعدیِ مالک — یکی

**#151 را ادغام کنید.** کوچک است (یک خط تریگر + untrack یک symlink)، ۷/۷ سبز، بدونِ migration، و پیش‌نیازِ اینکه #149، #153 و #155 واقعاً سنجیده شوند. بعد از آن، حفاظتِ `main` با چکِ `CI Gate`.

### موارد ناتمامِ این نوبت

- #152: فقط Preview با دادهٔ واقعی (از اینجا در دسترس نیست).
- #143/#144: CIِ ۰۹-۱۷ سبز؛ آزمونِ سرتاسریِ «دارایی → قیمت → هشدار → گیرندهٔ آزمایشی» با اعلانِ واقعی **اجرا نشد** — به migrationهای `phase31–33` روی محیطی با داده نیاز دارد. کانالِ واقعی خاموش ماند.
- #157: رندرِ صفحهٔ ادمین با نشستِ واقعی در ۳۶۰/۳۹۰/۱۴۴۰ دیده نشده؛ پروندهٔ سرتاسری با منبعِ واقعی و rubric خارج از این بسته.

---

## تحویل Codex — ۲۰۲۶-۰۹-۱۷ (سابقه)
...
| **B-055** | **پایانیِ هر جلسه در `symbol_history` زیرِ برچسبِ جلسهٔ بعد** — رله `trade_date` را از ساعتِ دیواری می‌گرفت و `hour12: false` نیمه‌شب را «24» می‌خواند | 🔴 CRITICAL | Relay / داده | ENGINEERING | 2026-09-22 | نمودار، NAV، بازده، P/E، بک‌تستِ تخصیص، هم‌ترازی با `index_history` | ادغامِ #153 → استقرارِ لیارا با **ثبتِ زمانِ دقیق** → تصمیمِ مالک دربارهٔ نمای تصحیح | **runtime:** روی Node 18.20.8 و 20.20.2 `en-CA`/`en-US` + `hour12:false` در ۰۰:۰۶ تهران «24» (h24) می‌دهد؛ Node 22 «00». **داده:** هر ۵۰٬۲۱۱ ردیف ساعتِ ۰۰ تهران نوشته شده (`captured_at` پیش‌فرضِ `now()`، رله تنظیمش نمی‌کند) ⇒ از نظرِ فیزیکی پیش از بازشدنِ بازارِ همان روز. **دسته‌بندیِ محتوا با زنجیرهٔ هر نماد** (`py = pl/(1+plp)` در برابرِ `close`ِ ردیفِ قبل): ۴۱٬۹۴۳ ردیف «یک جلسه جابه‌جا، زنجیره سالم» · ۳٬۱۰۵ برچسب روی **روزِ بی‌جلسه** (۰۸-۰۴، ۰۸-۱۲، ۰۸-۳۰) · ۳٬۱۰۵ **تکرارِ ۱۰۰٪ یکسان** همان جلسه پس از تعطیلی (۰۸-۰۵، ۰۸-۱۵، ۰۸-۳۱) · ۹۸۶ نخستین برچسب بی‌زنجیره · ۱٬۰۷۲ امروز. «سالم»: صفر ردیفِ `relay_eod`؛ کندل و `brsapi_history` تاریخِ منبع دارند | **FIX READY — PR #153** · ردیف‌های گذشته **اصلاح‌نشده** |
| **B-056** | **`standalone`ِ گزارش‌های تلفیقیِ کدال مبهم است — ممکن است رقمِ گروه باشد** | 🟠 HIGH | Codal / پارسر | ENGINEERING | 2026-09-22 | کارتِ بنیادی، فصل‌ها، P/E | تطبیقِ یک جفت با سندِ اصلیِ کدال (نمونه‌ها پایین)؛ سپس پارسر جدولِ شرکتِ اصلی را جدا بخواند | ۱٬۱۴۳ ردیفِ تلفیقی؛ `data.consolidated` در هیچ ردیفی پر نیست. در ۴۸ جفتِ هم‌دوره و هم‌حسابرسی، `standalone`ِ تلفیقی در ۳۳ بزرگ‌تر، ۱۱ برابر، ۴ کوچک‌تر از جداگانه (میانه ۱٫۰۵۲). پیش از #154، ۲۳۱ از ۴۸۲ کارت رقمِ تلفیقی را با تیکِ «اعتبارسنجی متقاطع» به شرکت نسبت می‌دادند. **نمونهٔ تطبیق:** فولاد سالانهٔ ۱۴۰۳ — جداگانه `LetterSerial=Txb1EjnrZUXYzQQQaQQQeLxoLN0g%3d%3d` (فروش ۲٬۷۶۳٬۵۵۵٬۷۱۳) در برابرِ تلفیقی `x9ljrUzOweknirW6EOOObOOO48xA%3d%3d` (۳٬۴۳۶٬۳۵۱٬۴۶۹، میلیون ریال)؛ در اکسلِ تلفیقی باید دید جدولِ اول «گروه» است یا «شرکتِ اصلی» | **OPEN** · INFERRED · مهارشده در #154 (جداگانه مقدم، تلفیقی با برچسبِ ابهام) |
| **B-057** | **بکاپِ ویندوز: CR انتهای رشتهٔ اتصال** و رمز در argvِ psql | 🟠 HIGH | Ops / بکاپ | ENGINEERING | 2026-09-22 | گیتِ بکاپ ← هر migrationِ Production | ادغامِ #155؛ سپس اجرای واقعی روی لپ‌تاپِ مالک تا `MANIFEST.txt` | CRLF با psql و Postgresِ واقعی بازتولید شد. `scripts/backup/pgurl.sh` حالا تنها مسیرِ هر دو اسکریپت است: CR، توقفِ exit 64 روی ورودیِ خالی/غیر URI، رمز به `PGPASSWORD`. خودِ ps1 با PowerShell 7.4 زیرِ pty آزموده شد (۵ حالت، رمزِ `p@ss w"o'rd$%:/#?`، هیچ نشتی جز `supabase db dump --db-url`ِ مستند). `backup-production-local.ps1`ِ Codex در هیچ شاخه‌ای نیست و مقایسه نشد | **FIX READY — PR #155** · PASSِ بکاپ **صادر نشده** · Windows PowerShell 5.1 اجرا نشد |
