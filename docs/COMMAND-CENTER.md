...
| **B-035** | **`fn_signals_hash_chain()` یک تابعِ `SECURITY DEFINER` است که `anon` می‌تواند از طریق `/rest/v1/rpc/` اجرا کند** | 🟠 HIGH | Portfolio / DB Security | ENGINEERING | 2026-07-31 | زنجیرهٔ هشِ `signals` | بازبینی: یا `REVOKE EXECUTE` از `anon`/`authenticated`، یا تبدیل به `SECURITY INVOKER` | منبعش `sql/terminal_t0.sql` است که روی Production **`APPLIED`** است — پس به احتمالِ زیاد Production هم همین را دارد. با `get_advisors` روی staging پس از اعمالِ fixtureِ پیش‌نیاز دیده شد (دو `WARN`). **هیچ اقدامی روی Production انجام نشد و نباید بدونِ بازبینی انجام شود** | **OPEN** · VERIFIED روی Staging · Production **INFERRED**، اندازه‌گیری نشده |
