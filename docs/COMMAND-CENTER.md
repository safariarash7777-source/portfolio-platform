...
| **B-053** | **هیچ استقرارِ خودکارِ رله روی لیارا از ۱۴۰۵/۰۵/۱۱ تا امروز موفق نبوده — ۴۰ از ۴۰ `failure`** | 🔴 CRITICAL | Relay / استقرار | **ARASH** (دسترسیِ لیارا) | 2026-09-14 | بستهٔ A · #122 · #126 · #127 — هیچ‌کدام روی رله زنده نیستند | کنسولِ لیارا را باز کن و لاگِ آخرین بیلد را بخوان. **یک فرضیهٔ قابلِ‌بررسی (تأییدنشده):** تنها `liara.json` مخزن در `relay/liara.json` است (`platform: node`, `port: 3400`)، ولی یکپارچه‌سازیِ GitHub→لیارا احتمالاً از **ریشهٔ مخزن** بیلد می‌کند، جایی که `package.json` مالِ اپِ Next است (`next start`، `engines >=20`) و هیچ `liara.json`ی نیست. اگر درست باشد، رفعش تنظیمِ مسیرِ اپ روی `relay/` در کنسول است یا افزودنِ `liara.json` در ریشه — **نه تغییرِ کد**. | اندازه‌گیریِ ۲۰۲۶-۰۹-۱۴ از GitHub Deployments API: محیطِ `arsadata`، ۴۰ رکوردِ اخیر از `6818b1e` (۲۰۲۶-۰۸-۰۲) تا `1d7e8f1` (امروز)؛ **۱۰ نمونهٔ بررسی‌شده همگی `failure`**. لینکِ لاگ `console.liara.ir` از محیطِ عامل مسدود است (۴۰۳ پراکسی). در برابرش، رله **زنده است** و می‌نویسد: `ir_market_history` امروز ۴ ردیف در ۲ ساعت گرفت. یعنی نسخهٔ در حالِ اجرا **قدیمی** است، نه متوقف | **OPEN** · VERIFIED |
