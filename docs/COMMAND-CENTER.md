...
| **B-015** | CI وجود دارد ولی **اجباری نیست** — یک PRِ قرمز هنوز می‌تواند merge شود | 🟠 HIGH | Portfolio / CI | **ARASH** | 2026-07-25 | کیفیتِ هر merge | **`RUNBOOK-branch-protection.md` نوشته شد** — مراحلِ دقیق. سرورِ GitHub MCP هیچ ابزارِ branch protection ندارد، پس این کار **فقط دستی** از کنسولِ گیت‌هاب ممکن است | workflow موجود و سبز است (`30194847587` روی `0023f6f`، `30276709105` روی `51ac8aa`) ولی هیچ required status check تنظیم نشده | **OPEN** · VERIFIED — از «بسته» به «باز» تصحیح شد: افزودنِ workflow ≠ اجباری‌شدنِ آن |
| ~~**B-016**~~ | ~~کامنتِ هشدارِ قیمت «هر ۵ دقیقه» می‌گوید، ولی `vercel.json` روزانه اجرا می‌کند~~ | 🟡 MEDIUM | Portfolio | ENGINEERING | 2026-07-25 | — | — | تصمیم: **متن به واقعیت اصلاح شد، زمان‌بندی دست نخورد** (متراکم‌کردنِ cron نیازمندِ ارتقای پلنِ Vercel است و تصمیمِ جدا می‌خواهد). کامنتِ `app/api/cron/alerts/route.ts` و عددِ `components/landing/ProductFacts.tsx` هر دو حالا «روزانه» می‌گویند، هم‌خوان با `vercel.json` (`0 6 * * *`) | ✅ **CLOSED 2026-07-29** |
...
|---|---|---|---|
| **شکافِ پرداخت → دسترسی** (یافتهٔ `E-01`ِ ممیزیِ Gate 0) | **FIXED** | `lib/entitlements.ts` (جدید) + فراخوانی در هر دو callback. پیش از این هیچ‌کدام از مسیرهای پرداخت در `entitlements` درج نمی‌کرد، در حالی که `lib/access.ts` سطحِ دسترسی را دقیقاً از همان جدول می‌خواند → کاربرِ پرداخت‌کرده `registered` می‌ماند. مدتِ دسترسی: ثابتِ `ENTITLEMENT_MONTHS` (مشاوره ۳ ماه · وبینار ۳ ماه، هم‌خوان با کامنتِ طراحیِ `phase11`). اعطا نسبت به `authority` **idempotent** است و شکستش redirect را نمی‌شکند. ۱۴ تست | 2026-07-29 |
| **زنجیرهٔ پرداختِ وبینار — هر سه حلقه شکسته بود** | **FIXED** | (۱) `webinars/register` تابعِ `register_for_webinar` (SECURITY DEFINER، متکی به `auth.uid()`) را با کلاینتِ **service-role** صدا می‌زد → `auth.uid()` تهی → هر ثبت‌نام شکست. حالا کلاینتِ کاربر. (۲) پاسخِ همان روت `registration_id` می‌داد ولی `app/webinars/page.tsx:115` دنبالِ `registration.id` بود → وبینارِ پولی **هرگز به درگاه نمی‌رفت**؛ پاسخ نرمال شد. (۳) `webinars/payment` ستون‌های ناموجودِ `amount_toman`/`description` را درج می‌کرد (جدول `amount` دارد) و `authority` را ذخیره نمی‌کرد؛ `requestPayment` هم callbackِ دوره را می‌فرستاد. بازنویسی شد: authority اول، سپس `create_payment` RPC با کلاینتِ کاربر، با callbackِ اختصاصی. (۴) callback مقدارِ `status:'verified'` می‌گذاشت که `CHECK (pending\|paid\|failed)` را نقض می‌کرد؛ حالا از `verify_payment`/`fail_payment` RPC با بایندِ authority و کوتاه‌مدارِ idempotency استفاده می‌کند (کدِ ۱۰۱ زرین‌پال قابلِ replay بود) | 2026-07-29 |
