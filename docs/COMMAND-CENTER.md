...
| ~~**B-015**~~ | ~~مخزن **هیچ GitHub Actions workflowی ندارد**~~ | 🟡 MEDIUM | Portfolio / CI | ARASH | 2026-07-25 | — | باقی‌مانده: فعال‌کردنِ **branch protection** روی `main` با چکِ لازمِ `CI Gate` — کارِ کنسولِ گیت‌هاب است و از داخلِ مخزن انجام نمی‌شود | `.github/workflows/ci.yml` اضافه شد: ۴ job (dependencies · quality · security-and-sql · CI Gate) پوشش‌دهندهٔ npm ci، audit، typecheck، lint، test:core، test:calc، build، اسکنِ سکرت و اعتبارسنجیِ SQL | ✅ **CLOSED 2026-07-25** (P1-010) — با یک اقدامِ دستیِ باقی‌مانده |
