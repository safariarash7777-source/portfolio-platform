...
|---|---|---|---|
| **A — توقفِ نوشتنِ تاریخچهٔ بی‌خواننده** | فقط استقرارِ رله. **بکاپ لازم ندارد** — دیتابیس لمس نمی‌شود، بازگشت یک متغیرِ محیطی است | پس از ۲۴ ساعت: `stocks`/`funds` → `rows_24h = 0` و `newest` ثابت · `gold`/`currency` → `rows_24h ≈ 40` و `newest` تازه · `/market` روندِ طلا و دلار را بدهد. جزئیات: [`ops/RELEASE-history-sections.md`](./ops/RELEASE-history-sections.md) | **کد آماده (#121)** · منتظرِ استقرار |
| **B — مهارِ `create_payment`** (`phase30`) | فقط مجوزِ اجرای یک `REVOKE`. **بکاپ لازم ندارد**؛ بازگشت یک `GRANT`. ⚠️ **ترتیب اجباری:** اول استقرارِ برنامه، بعد `REVOKE` — برعکسش ۵۰۰ِ مبهم می‌دهد | `auth_x = f` · `anon_x = f` · `svc_x = t` · روتِ پرداخت ۵۰۳ بدهد نه ۵۰۰. ⚠️ **این «توقفِ کنترل‌شدهٔ پرداخت» است، نه حفظِ قابلیت:** `svc_x = t` کارکردن نیست (تابع نشست‌محور است و زیرِ `service_role` بدونِ claims خودش رد می‌کند)، پس تا #113 پرداختِ تازه ممکن نیست. امروز هزینه‌اش صفر است (`payments` صفر ردیف). جزئیات و اثرِ `authority`ِ بی‌رکورد: [`ops/RELEASE-payment-containment.md`](./ops/RELEASE-payment-containment.md) | **کد آماده (#121)** · منتظرِ مجوز |
...
| **D — بکاپِ اثبات‌شدهٔ Production** | ماشینی با Docker + رمزِ دیتابیس دستِ اپراتور | `MANIFEST.txt` با `restore exit: 0` **و** `result: PASS`. ⚠️ **`PARTIAL` مجوز نمی‌دهد:** کدِ ۲ یعنی ساختار تأیید شد ولی برابریِ دادهٔ جدول‌های زنده **اثبات نشد** — شمارشِ داخلِ بازه شاهدِ drift است، نه اثبات. تطبیق با snapshotِ مشترکِ dump هنوز انجام نمی‌شود (عبورِ `--snapshot` از Supabase CLI **نامشخص**). و محدودهٔ تضمین: شمارشِ ردیف محتوا را نمی‌بیند و `sha256` فقط تمامیتِ فایل است | **مسدود — دستِ آرش** |
