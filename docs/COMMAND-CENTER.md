...
| **B-053** | **یکپارچه‌سازیِ GitHub→Liara برای اپِ `arsadata` هرگز یک استقرارِ موفق نداشته** | 🔴 CRITICAL | Relay / استقرار | **ARASH** (کنسولِ Liara) | 2026-09-14 | بستهٔ A · #122 · #126 · #127 — هیچ‌کدام روی رله زنده نیستند | **۱)** لاگِ آخرین بیلدِ ناموفق را بفرست: `console.liara.ir/apps/arsadata/repository-deployment-failures/6a58bb221ec80c5d48b608e3/6aa7b422db7d526813457893` (فقط متنِ خطا). **۲)** شش تنظیمِ کنسول: مخزن/branch · پوشهٔ build · platform · دستورِ start · port · نسخهٔ Node. **۳)** مسیرِ میان‌بر که به رفعِ یکپارچه‌سازی نیاز ندارد: استقرارِ دستی از پوشهٔ `relay/`. جزئیات و پذیرش: [`RUNBOOK-relay-deploy.md`](./RUNBOOK-relay-deploy.md) | **۱۵۶ رکوردِ استقرار** از ۲۰۲۶-۰۷-۱۶ تا امروز؛ **۱۶ نمونهٔ پخش‌شده در کلِ بازه — از اولین رکورد تا آخرین — همگی `failure`، صفر موفقیت**. پس رگرسیون نیست؛ پیکربندی از روزِ اول کار نکرده و هر نسخه‌ای که تا امروز روی رله رفته از مسیرِ دیگری رفته و **شناسه‌اش ثبت نشده**. ⚠️ **علت هنوز قطعی نیست** — لاگ خوانده نشده. فرضیهٔ «بیلد از ریشهٔ مخزن» تأییدنشده است. **آنچه اثبات شد:** artifact سالم است — `node relay/server.mjs` روی `main@1d7e8f1` بالا می‌آید و `/healthz` → `ok`؛ رله **هیچ وابستگیِ خارجی ندارد** (فقط `node:*`)، پس نه lockfile لازم است نه `npm ci`. دسترسیِ لاگ از محیطِ عامل صفر است: `console.liara.ir`/`api.iran.liara.ir`/`liara.ir` هر سه `000`، و باتِ `liara-cloud` نه `description` می‌فرستد نه check-run | **OPEN** · VERIFIED |
...
|---|---|---|---|
| **A — توقفِ نوشتنِ تاریخچهٔ بی‌خواننده** | استقرارِ رله روی لیارا | ⛔ **مسدود — خطِ استقرارِ رله شکسته است** (`B-053`). کد در `main` (`1d7e8f1`) هست ولی رله هنوز نسخهٔ قدیمی را اجرا می‌کند. شاهدِ مبنا (۲۰۲۶-۰۹-۱۴ ۰۸:۴۷ UTC، پیش از هر استقرار): هر چهار بخش در ۲ ساعتِ گذشته ۴ ردیف گرفتند و `newest` هر چهار `08:37:31Z` است — یعنی `stocks`/`funds` هنوز نوشته می‌شوند | **مسدود** |
