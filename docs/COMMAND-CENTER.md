...
## وضعیت جاری — ۱۴۰۵/۰۶/۳۱ (۲۰۲۶-۰۹-۲۲) · `P2-DELIVERY-CLOSURE-001`
...
| **#153** رله: برچسبِ جلسه (B-055) | `734191b` | #151 | ۷/۷ ✅ | آمادهٔ بازبینی | پس از ادغام، استقرارِ لیارا جدا لازم است؛ هزینهٔ گذار پایین |
| **#155** بکاپ: CR در رشتهٔ اتصال (B-057) | `522114c` | #151 | در حالِ اجرا | آمادهٔ بازبینی | PASSِ بکاپ فقط با اجرای واقعی روی لپ‌تاپ |
| **#150** تقدمِ اصلاحیه | `24beb20` | `main` | ۷/۷ ✅ | آمادهٔ بازبینی | **بدونِ #154 ناقص است** |
| **#154** فصل‌ها + زمانِ انتشار | `a3ea0fe` | #150 | ۷/۷ ✅ | آمادهٔ بازبینی | B-056 حل نشده |
| **#144** دارایی و مقایسه | `48cea22` | `main` | ۷/۷ ✅ (۰۹-۱۷) | آمادهٔ بازبینی | `phase32`؛ بازبینیِ قراردادِ هدف (تحویلِ Codex) |
| **#149** هشدار | `09d4893` | #144 | **هرگز اجرا نشده** — فقط ۲ چکِ پیش‌نمایش | **آمادهٔ بازبینی نیست** | اصلاحِ تریگر به‌صورتِ `7e6fd47` محلی آماده است؛ push به این شاخه را محیط رد کرد (پایین) |
...
### ظرفیتِ دیتابیس — اندازه‌گیریِ امروز

`1295 MB` (بدونِ تغییر). رشدِ فعلی **حدودِ ۵٫۵ MB در هفته** — مسئله انباشته است، نه نرخ.

| جدول | کل | جدول/TOAST | index | ردیف | رشدِ هفتگی | خواننده |
|---|---|---|---|---|---|---|
| `symbol_history` | 756 MB | 503 | 252 | ۲٫۱۲M | ~۶٬۲۵۰ ردیف | نمودار، NAV، P/E |
| `ir_market_history` | 488 MB | 488 | 0.9 | ۱۱٬۲۳۶ | ~۵۵۰ ردیف | فقط `gold,currency` |
| `codal_reports` | 19 MB | 17 | 2 | ۹٬۴۳۹ | ~۸۷۰ | کارت |

**B-052 هفت روز بعد هم برقرار است:** `stocks` و `funds` از ۰۹-۱۵ ۰۹:۴۲ صفر ردیف؛ `gold`/`currency` تا دیشب.

**طرحِ صرفه‌جویی** (هیچ‌کدام اجرا نشده؛ هر کدام بکاپِ معتبر + مجوزِ جدا می‌خواهد):

| اقدام | صرفه | برگشت‌پذیری | ریسک |
|---|---|---|---|
| `DROP INDEX idx_symbol_history_symbol_date` — همان ستون‌هایِ کلیدِ یکتا، فقط `DESC`؛ btree هر دو جهت را می‌خواند | **۹۵ MB** | کامل (`CREATE INDEX`) | تغییرِ برنامهٔ پرس‌وجو؛ پیش از حذف با `EXPLAIN` روی پرس‌وجوهای صفحهٔ نماد سنجیده شود |
| آرشیوِ `ir_market_history` بخشِ `stocks`,`funds` به فایلِ بیرونی، سپس حذف | **۴۵۹ MB** | فقط از فایلِ آرشیو | **هیچ خواننده‌ای در کد نیست** (`lib/core/trend.ts` فقط `in.(gold,currency)`). `DELETE` به‌تنهایی حجمِ فیزیکی را کم نمی‌کند؛ `VACUUM FULL` قفلِ کامل می‌گیرد |
| — | **مجموع ≈ ۵۵۴ MB (۴۳٪)** | | |

`symbol_history` کوتاه نشود: نمودار و P/E تاریخچهٔ بلند (از ۱۳۸۰) لازم دارند. `raw` ۵۷٪ حجمِ ردیف است ولی خوانندهٔ NAV به آن وابسته است.
...
| **B-055** | **پایانیِ هر جلسه در `symbol_history` زیرِ برچسبِ جلسهٔ بعد ذخیره شده** — رله `trade_date` را از ساعتِ دیواری می‌گرفت و `hour12: false` نیمه‌شب را «24» می‌خواند | 🔴 CRITICAL | Relay / داده | ENGINEERING | 2026-09-22 | نمودار، NAV، بازده، مقایسهٔ سبد، هم‌ترازی با `index_history` | ادغامِ #153 → استقرارِ لیارا با **ثبتِ زمانِ دقیق** → تصمیمِ مالک دربارهٔ نمای تصحیحِ ردیف‌های گذشته | هر ۵۰٬۲۱۱ ردیفِ `relay_eod` ساعتِ ۰۰ تهران؛ `fx_rates` ۶۷ از ۶۸ ساعتِ ۰۰؛ `index_history`/`market_breadth` با کلیدِ تاریخِ منبع ساعتِ ۱۴ و سالم. سهمِ مثبت‌ها با یک برچسب جابه‌جایی ۵ واحد فاصله، بی‌جابه‌جایی ۲۱. در روزِ استقرار پایانیِ یک جلسه ذخیره نمی‌شود (کلیدِ یکتای بی‌`source`) | **FIX READY — PR #153** · ردیف‌های گذشته **اصلاح‌نشده** |
| **B-056** | **`standalone`ِ گزارش‌های تلفیقیِ کدال احتمالاً رقمِ گروه است، نه شرکت** | 🟠 HIGH | Codal / پارسر | ENGINEERING | 2026-09-22 | کارتِ بنیادی، فصل‌ها، P/E | تطبیقِ یک گزارشِ تلفیقی با سندِ اصلیِ کدال؛ سپس یا پارسر جدولِ شرکتِ اصلی را بخواند یا تلفیقی از رقابتِ `dedupeByPeriod` بیرون رود | ۱٬۱۴۳ ردیفِ تلفیقی؛ در ۱۹۳ از ۱۹۷ دورهٔ دارای هر دو نوع، تلفیقی برنده؛ ۵۵۰ دوره فقط تلفیقی. `standalone`ِ دو نوع در یک دوره برابر نیست. پارسر (`relay/codal.mjs`) اولین جدولِ سود و زیانِ مطابق را برمی‌دارد | **OPEN** · INFERRED — `codal.ir` از این محیط در دسترس نیست |
| **B-057** | **بکاپِ ویندوز: CR انتهای رشتهٔ اتصال** — `database "postgres\r" does not exist` | 🟠 HIGH | Ops / بکاپ | ENGINEERING | 2026-09-22 | گیتِ بکاپ ← هر migrationِ Production | ادغامِ #155؛ سپس اجرای واقعی روی لپ‌تاپِ مالک تا `MANIFEST.txt` | بازتولید با psql و Postgresِ واقعی و بایت‌های CRLF؛ LF سالم. همچنین رمز در argvِ psqlِ داخلِ کانتینر (با `ps` دیده شد) — مستند، رفع‌نشده | **FIX READY — PR #155** · PASSِ بکاپ **صادر نشده** |
