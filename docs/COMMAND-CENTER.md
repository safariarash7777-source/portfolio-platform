...
| **B-048** | **`Tsetmc/Index.php` روی مسیرِ قدیمی هیچ شمارنده‌ای نداشت** — پرمصرف‌ترین چرخهٔ روز (هر ۵ دقیقه) با پرچمِ خاموش از بودجه **نامرئی** بود، در حالی که جدولِ `BRSAPI-CLIENT-DESIGN` ادعا می‌کرد شمرده می‌شود | 🟠 HIGH | Relay / سهمیه | ENGINEERING | 2026-09-12 | سقفِ روزانهٔ BrsApi | — | شاخهٔ `else` در `relay/server.mjs` (وقتی کلاینت ساخته نشده) `countLegacy` نداشت؛ پرچم امروز در Production **خاموش** است. سند و کد یکی نبودند و سند باور شده بود | ✅ **CLOSED 2026-09-12** — شمارنده اضافه شد؛ گاردِ `relay/brsapi-meter.test.mjs` فهرستِ نقاطِ تماس را **قرارداد** کرد. **آزمونِ نقص:** برداشتنِ یک شمارنده ۳ تست را می‌اندازد |
| **B-049** | **در حالتِ `enforce` بدونِ انبارِ ماندگار، مصرف نامحدود بود** — `LegacyMeter` آن را `unmetered` می‌شمرد و **عبور می‌داد**، یعنی دقیقاً در حالتی که هدفش سقف گذاشتن است سقفی نبود | 🟠 HIGH | Relay / سهمیه | ENGINEERING | 2026-09-12 | فعال‌سازیِ `BRSAPI_BUDGET_ENFORCE_LEGACY` | — | ترکیبِ «پرچمِ اجرا روشن + `SUPABASE_URL` غایب». چهار تستِ ماتریسِ حالت در `relay/brsapi-legacy-meter.test.mjs` | ✅ **CLOSED 2026-09-12** — سقفِ درون‌فرایندی. با restart صفر می‌شود، پس تضمینِ روزانه **نیست** و همین در `/debug → brsapiLegacy.fallback` نوشته می‌شود |
| **B-050** | **۶۹ نماد از ۱٬۱۳۴ آخرین روزِ معاملاتی را ندارند و مثلِ نمادِ زنده رندر می‌شوند** — قدیمی‌ترین `2018-10-29` | 🟠 HIGH | Portfolio / داده | ENGINEERING (موتور ✅) · ARASH (اجرای `phase29`) | 2026-09-12 | هر صفحه‌ای که قیمتِ نماد نشان می‌دهد | اجرای `sql/phase29_symbol_liveness.sql` پس از دروازهٔ بکاپ (`C`) | اندازه‌گیریِ زندهٔ ۱۴۰۵/۰۶/۲۱ روی Production: ۱٬۰۶۵ زنده · ۶۹ عقب. **تفکیک با تابلوی زنده:** ۲۱ تا روی تابلو هستند ⇒ عقب‌ماندگیِ **لولهٔ ما**؛ ۴۸ تا نیستند ⇒ متوقف/حذف‌شده؛ هیچ‌کدام از ۶ نمادِ بیش‌از‌یک‌سال روی تابلو نیستند | **موتور آماده** (`lib/core/symbolLiveness.ts`، ۲۱ تست) · **`phase29` NOT_APPLIED** · تا اجرا، لایهٔ داده `rpc_missing` می‌گوید نه «همه زنده» |
...
| **#121** | فهرستِ توانمندی + نمونهٔ طراحی + (تازه) بودجه و زندگیِ نماد | `304194e` | `main` | در جریان | همین جلسه دو بستهٔ کد به آن اضافه شد | بازبینی |
...
> **بازنویسیِ ۱۴۰۵/۰۶/۲۱.** فهرستِ قبلی از `P2-G1-001` می‌آمد و بخش بزرگی از آن
> ماه‌ها پیش انجام شده بود. ترتیب اینجا از `MARKET-CORE-AUDIT` §۳ می‌آید:
> **اول چیزی که هر روزِ تأخیرش هزینهٔ برگشت‌ناپذیر دارد.**

| # | Action | Owner | Dependency | Expected Output |
|---|---|---|---|---|
| **۱** | اجرای بکاپِ Production طبقِ `RUNBOOK-backup-windows.md` روی ویندوزِ آرش | **ARASH** | — | فایلِ بکاپ + **بازیابیِ آزمایشیِ موفق** در مقصدِ ایزوله. این دروازهٔ ۲ تا ۵ است |
| **۲** | استقرارِ رله با گامِ ۱ سندِ #119 (بستنِ نشتیِ `ir_market_history`) | **ARASH** (استقرار) | — | تنها کارِ فهرست که **بدونِ بکاپ هم مجاز است** (دیتابیس دست نمی‌خورد). هر روزِ تأخیر داده‌ی بی‌مصرفِ برگشت‌ناپذیر می‌سازد |
| **۳** | استقرارِ رلهٔ حاوی پوششِ کاملِ بودجه (`R-013`, `R-014`) با پرچم‌های **دست‌نخورده** | **ARASH** (استقرار) | ۲ | `/debug → brsapiLegacy.used` باید ۱۲ producer نشان دهد و `hook.unregistered` خالی باشد. یک روز مشاهده، بعد تصمیمِ `enforce` |
| **۴** | اجرای `phase20`→`phase25` سپس `phase29` سپس `phase23` (آخر) | **ARASH** (مجوز) | ۱ | ترتیب اجباری است (`MIGRATION-LEDGER` + `PRODUCTION-ACTIVATION` بندِ ۹). `phase23` باید **آخرین** باشد وگرنه جدول‌های تازه پوشش نمی‌گیرند |
| **۵** | merge شدنِ #113 (پرداخت → دسترسی) | ENGINEERING | ۴ | تا قبل از آن، حفرهٔ مبلغِ جعلی روی Production **باز** است (اندازه‌گیریِ ۱۴۰۵/۰۶/۲۱). بستهٔ انتشار: `docs/ops/RELEASE-113-PAYMENT.md` |
| **۶** | بستنِ #91 | ENGINEERING | ۵ | دو مسیرِ موازیِ پرداخت نباید باز بماند |
| **۷** | فعال‌کردنِ پروژهٔ staging (`B-047`) | **ARASH** | — | بدونِ آن هیچ migrationی «تمرین‌شده» نیست و گامِ ۴ ریسکِ غیرلازم دارد |
| **۸** | تصمیمِ `D-026` (منبعِ ترکیبِ داراییِ صندوق) و `D-031` (طرحِ ورودِ اعضا) | **ARASH** | — | `R-005` و کلِ پنلِ عضو پشتِ این دو هستند |
| **۹** | تصمیمِ `D-025` (احرازِ کانکتورِ Vercel) | **ARASH** | — | تا آن روز هیچ ادعایی دربارهٔ استقرار و لاگِ Production از این محیط اثبات‌پذیر نیست |
| **۱۰** | بازبینیِ نمونهٔ طراحی (§۱۳) | **ARASH** | — | بدونِ آن `MC-01`…`MC-03` شروع نمی‌شوند |

**مسیرِ مستقلِ Mini App** (مسدودکنندهٔ بالا نیست): تعیینِ مالک → تهیهٔ VPS و
...
| **R-013** | **هر تلاشِ واقعیِ BrsApi باید در یک بودجه شمرده شود، مستقلِ از هر پرچم** | `OPS-01` | هر ۱۲ نقطهٔ کلیدِ اصلی در هر دو حالتِ پرچم؛ `Market/Commodity.php` عمداً بیرون (کلیدِ جدا) | `relay/brsapi-meter.test.mjs` — فهرستِ نقاط **قرارداد** است و endpointِ ثبت‌نشده تست را می‌اندازد · `test:relay` ۱۴۷ سبز · `/debug → brsapiLegacy.hook` | ✅ **DONE** (کد) · فعال‌سازیِ `enforce` تصمیمِ مالک است |
| **R-014** | **حالتِ `enforce` نباید با نبودِ انبار به مصرفِ نامحدود تبدیل شود** | `OPS-01` | ماتریسِ (پرچم × اجرا × انبار) کامل تست شده باشد | ۴ تستِ تازه در `relay/brsapi-legacy-meter.test.mjs` · تنزل در `/debug → brsapiLegacy.fallback` دیده می‌شود | ✅ **DONE** (کد) |
| **R-015** | **نمادی که دیگر معامله نمی‌شود نباید مثلِ نمادِ زنده دیده شود** | `DATA-01` | پنج حالتِ جدا (`live`/`lagging`/`off_board`/`undetermined`/`never_seen`)؛ «بازارِ بسته» از «فیدِ خراب» جدا؛ تا اجرای `phase29` پاسخ «نمی‌دانم» باشد نه «زنده» | `lib/core/symbolLiveness.ts` + ۲۱ تست · `lib/core/symbolLiveness.integration.test.ts` ۸ تست روی Postgres 16.13 واقعی · `sql/phase29_symbol_liveness.sql` | **موتور ✅ · `phase29` NOT_APPLIED (`B-050`)** |
