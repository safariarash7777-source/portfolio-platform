...
|---|---|---|---|---|---|---|---|
| **B-031** | ستونِ `codal_reports.published_at` برای **هر ۷٬۸۰۵ ردیف** `NULL` است | 🟠 HIGH | Portfolio / داده | ENGINEERING | `R-011`، هر ادعای «تازگیِ گزارش» | `count(*) filter (where published_at is null)` = ۴٬۸۰۰ (ن‑۳۰) + ۳٬۰۰۵ (ن‑۱۰) = کلِ جدول | **OPEN** · VERIFIED |
| **B-032** | **ترکیب دارایی صندوق‌ها هیچ منبع و هیچ جدولی ندارد** | 🟠 HIGH | Portfolio / داده | ARASH (تصمیمِ منبع) | `R-005`، صفحهٔ صندوق | `list_tables` روی Production: ۴۱ جدول، هیچ‌کدام ترکیبِ دارایی ندارد؛ `lib/market-ir.ts` فقط قیمت/NAV/خالص دارایی می‌آورد | **OPEN** · VERIFIED |
| **B-033** | پروژهٔ **staging** (`oqjcvkzyvhqnphopedpn`) وضعیتش `INACTIVE` است | 🟠 HIGH | Portfolio / زیرساخت | ARASH | `G2-006`، هر تمرینِ migration، `D-001` | `list_projects` (۲۰۲۶-۰۹-۰۶) → `"status":"INACTIVE"` | **OPEN** · VERIFIED |

> **چرا `B-031` مهم است:** صفحهٔ نماد نمی‌تواند بگوید «این گزارش کِی منتشر شد» — فقط می‌تواند بگوید
...
| **D-026** | منبعِ ترکیب دارایی صندوق‌ها چه باشد؟ | ARASH | 2026-09-06 | `R-005`، `B-032` | کدال (صورت‌وضعیت پرتفوی) / منبع ثالث / صرفِ‌نظر | پیش از شروعِ `MC-02` | **OPEN** |
...
| خواندنِ Supabase **Production** | ✅ آزموده | ادعاهای داده قابلِ راستی‌آزمایی‌اند — سه بلاکرِ `B-031`…`B-033` از همین راه پیدا شدند |
...
| **staging** Supabase | ❌ `INACTIVE` (`B-033`) | تمرینِ migration ممکن نیست ⇒ `G2-006` جلو نمی‌رود |
