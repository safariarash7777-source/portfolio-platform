...
| **B — مهارِ `create_payment`** (`phase30`) | فقط مجوزِ اجرای یک `REVOKE`. **بکاپ لازم ندارد** — نه جدول، نه ستون، نه ردیف، نه امضا؛ بازگشت یک `GRANT` | `has_function_privilege('authenticated', 'public.create_payment(integer,text)', 'EXECUTE') = false` و `service_role = true` · مسیرِ `/api/payment/request` به‌جای ۵۰۰، ۵۰۳ صادق بدهد | **کد آماده (#121)** · منتظرِ مجوز |
...
| **D — بکاپِ اثبات‌شدهٔ Production** | ماشینی با Docker + رمزِ دیتابیس دستِ اپراتور | `MANIFEST.txt` با `restore exit: 0`، `result: PASS`، sha256ِ سه فایل، و `comparison.txt` بدونِ `missing`/`extra`/`changed` (ردیف‌های `drift` مجازند) | **مسدود — دستِ آرش** |
