...
## تحویل Claude — ۲۰۲۶-۰۹-۱۷ (ب) · سخت‌کردن #149

**محدودهٔ این نوبت:** فقط قابل‌اعتمادکردن #149؛ قابلیت تازه اضافه نشد.
**شاخهٔ مسئول:** `claude/rebalance-alert-delivery`. شاخهٔ #144 لمس نشد.
**ترتیب ادغام (اجباری):** #144 اول، #149 بعد. #149 روی سرشاخهٔ #144 بنا شده
چون به `decideDeviationAlert` و `view.ts` تکیه دارد که فقط آنجا هستند.

### باگی که در کدِ خودم پیدا شد — هشدارِ گم‌شده

ترتیبِ قبلی «ادعا ← ارسال ← ثبت» بود. اگر پردازش **بین ارسالِ موفق و ثبتِ آن**
می‌مرد، رویداد با صفر تلاشِ ثبت‌شده می‌ماند و هر اجرای بعدی `in_flight` می‌گرفت:
یعنی آن هشدار **برای همیشه مسدود** می‌شد. وجودِ `attemptsRecorded` به‌تنهایی
این را نمی‌گرفت، دقیقاً همان‌طور که بازبینی گفته بود.

اصلاح: ترتیب حالا **«اول ثبت، بعد ارسال»** است. ردیفِ `pending` پیش از تماس با
کانال نوشته می‌شود، و **همان نوشتن خودش قفل است** — قیدِ `rad_attempt_once`
اجازه نمی‌دهد دو پردازش همان شمارهٔ تلاش را آغاز کنند. تکیه به «اول بخوان بعد
بنویس» این تضمین را نمی‌داد.

### پنج حالتِ قطعِ کار — چه می‌ماند و ادامه چطور است

| حالت | در دیتابیس چه می‌ماند | ادامه |
|---|---|---|
| مرگ پس از ثبتِ آغاز، پیش از ارسال | یک ردیفِ `pending` بدونِ نتیجه | تا پایانِ اجاره «دستِ رقیب»؛ بعد `unknown` ثبت و کار از سر گرفته می‌شود |
| گیرنده گرفت ولی پاسخ گم شد | همان `pending` | همان مسیر — و چون **نمی‌دانیم** رسید یا نه، نه `sent` جعل می‌شود نه `failed` |
| شکستِ قطعی، سپس تلاشِ مجدد | `pending` + `failed` با دلیل | تلاشِ بعدی با شمارهٔ بعدی، تا سقفِ کل |
| دو پردازشِ هم‌زمان | یک رویداد، یک `pending` | بازندهٔ قید عقب می‌کشد و `in_flight` می‌گیرد |
| ری‌استارت با کارِ نیمه‌تمام | هرچه ثبت شده بود | وضعیت از دیتابیس بازخوانده می‌شود؛ شمارهٔ تلاش ادامه پیدا می‌کند |

`unknown` یک وضعیتِ کامل در دیتابیس است و قیدش دلیل می‌خواهد
(`rad_unknown_needs_why`). سقفِ `maxTotalAttempts` جلوی تلاشِ بی‌پایان را
می‌گیرد و رویداد وضعیتِ پایانیِ **قابلِ دیدن** پیدا می‌کند.

**محدودیتی که ادعا نمی‌شود:** نه Resend و نه تلگرام ضدتکرار تضمین نمی‌کنند.
وقتی یک تلاشِ رها `unknown` می‌شود و دوباره تلاش می‌شود، **احتمالِ پیامِ
تکراری واقعی است**. رسیدگی: رویداد و همهٔ تلاش‌هایش در دفتر می‌مانند و قابلِ
شمارش‌اند؛ اگر روزی لازم شد، راهِ درست شناسهٔ ایده‌مپوتنسی در خودِ کانال است،
نه سخت‌ترکردنِ حدس‌های ما.

### چه چیزی هشدار را اجرا می‌کند — **بررسی دستی**

از کد بررسی شد: هیچ‌چیز به‌صورت خودکار `POST /api/portfolio/rebalance-alert`
را صدا نمی‌زند (`grep` روی `app`، `lib`، `components`، `relay`، `.github`،
`scripts`: صفر ارجاع). نه ثبتِ دارایی، نه تغییرِ قیمت، نه cron.

پس وضعیتِ امروز صریحاً **«بررسی دستی»** است. رندرِ صفحه و تازه‌سازی هیچ
ارسالی نمی‌سازند، چون مسیر `POST` است و صفحه صدایش نمی‌زند. پایشِ خودکار
**بخشی از این بسته نیست** و اگر لازم شد، تصمیم و بستهٔ جداگانه می‌خواهد.

### واحدِ گزارش — بدونِ قطعیتِ کاذب

سربرگ و استنباط حالا **جدا** ثبت می‌شوند (`unit_header`، `unit_inferred`)، چون
سازگاریِ «مقدار × نرخ» یک شاهدِ کمکی است نه اثباتِ مستقل: اگر ستونِ نرخ خودش
مقیاسِ دیگری داشته باشد، همان رابطه با مقیاسِ غلط هم جور درمی‌آید. `basis`
چهار حالت دارد: `header+arithmetic` · `header_only` · `arithmetic_only` ·
`conflict`/`unresolved` (هر دو با `unit=null`). `qty_unit` صریح `null` است تا
واحدِ مقدارِ کالا با واحدِ مبلغ قاطی نشود.

دربارهٔ شپنا: در اسناد جست‌وجو شد و **هیچ ادعای «تجدید ارائه تأییدشده»**
وجود ندارد؛ چیزی برای اصلاح نبود و چیزی هم اضافه نشد.

### بند ۴ — تلاش تکراری نشد

محدودیتِ شبکه همان است (پراکسی `403 CONNECT` به رجیستریِ کانتینر). دورزدنش را
امتحان نکردم. به‌جایش `scripts/qa-member-holdings-loop.mjs` نوشته شد: مسیرِ
ورود ← ثبت ← ذخیره ← نشستِ تازه ← اصلاح ← هشدارِ آزمایشی، با دو عضو و کنترلِ
جداییِ اطلاعات. بدونِ متغیرهای محیطی با کدِ ۲ و فهرستِ دقیقِ آنچه کم است خارج
می‌شود، و اگر `REBALANCE_ALERT_CHANNELS` چیزی جز `log` باشد **اجرا نمی‌شود** —
یک آزمونِ QA نباید به صندوقِ ورودیِ کسی برسد. هیچ کلیدی در کد یا چت نیست.

### شاهد آزمون روی آخرین تغییر همین شاخه

`test:core` **۱۱۶۱** · `test:db` **۳۷۷** روی Postgres 16.13 واقعیِ محلی
(`CI=true`، صفر skip) · `test:relay` · `typecheck` · `lint:ci` · `build` ·
`scan:secrets` — همه سبز. کفِ `MIN_PASS` به ۳۷۷ رسید.

`phase33` روی Production **اجرا نشده** و پشتِ همان دروازهٔ بکاپ/بازیابی است.

- **نقطه ادامه:** بازبینی مستقل روی سرشاخهٔ `claude/rebalance-alert-delivery`؛ سپس ادغامِ #144 و بعد #149؛ آزمون مرورگر به‌محضِ فراهم‌شدنِ محیط.

## تحویل Claude — ۲۰۲۶-۰۹-۱۷ · اتصال پایدارِ هشدار بازتوازن

**محدودهٔ این نوبت:** فقط مسیرِ اجراییِ هشدار. **شاخهٔ مسئول:**
`claude/rebalance-alert-delivery` (منشعب از سرشاخهٔ #144). شاخهٔ #144 دست‌نخورده
ماند تا بازبینی‌اش مستقل بماند. پیش از شروع، `main`، سرشاخهٔ #144 و کامنت‌های
#144/#145 دوباره خوانده شدند: کسی این کار را نگرفته بود.

- **موتورِ موازی ساخته نشد.** همان `decideDeviationAlert` صدا زده می‌شود و ورودی‌اش دقیقاً همان `RebalanceResult` است که صفحه نشان می‌دهد (`view.ts` حالا نتیجهٔ کامل را هم برمی‌گرداند). اگر مسیرِ هشدار خودش دوباره حساب می‌کرد، می‌شد وضعیتی که صفحه «پوشش ناقص» بگوید و هشدار برود.
- **ذخیرهٔ پایدار:** `sql/phase33_rebalance_alerts.sql` — دو جدول، چون «تصمیم گرفتیم هشدار بدهیم» و «پیام واقعاً رفت» دو واقعیتِ جدا هستند. `rebalance_alert_events` تصمیم را با `UNIQUE (user_id, alert_key)` قفل می‌کند و `rebalance_alert_deliveries` **هر تلاش** را ثبت می‌کند. هر دو append-only با `deny_mutation()`، و `service_role` عمداً `TRUNCATE`/`DELETE`/`UPDATE` ندارد (درسِ phase20: بدونِ آن، گاردِ append-only توخالی است).
- **«شکست، موفقیت نیست» در سطحِ دیتابیس بسته شد:** قیدِ `rad_sent_needs_time` و `rad_failed_needs_why`. ردیفِ `sent` بدونِ زمان و `failed` بدونِ دلیل اصلاً ذخیره نمی‌شود. `skipped` ایمیل (کلید تنظیم نشده) هم موفقیت حساب نمی‌شود.
- **قفلِ هم‌زمانی در دیتابیس است، نه در یک `if`:** `INSERT … ON CONFLICT DO NOTHING RETURNING id`. بازندهٔ مسابقه شناسه نمی‌گیرد و ارسال نمی‌کند. **یک باگِ واقعی همین‌جا پیدا و بسته شد:** قابلیتِ «تلاشِ دوبارهٔ ارسالِ شکست‌خورده» ابتدا ضدتکرار را خنثی می‌کرد، چون دو اجرای هم‌زمان هر دو می‌دیدند «ارسالِ موفقی نیست». حالا `attemptsRecorded` تفاوتِ «رقیبِ در حالِ اجرا» و «شکستِ ثبت‌شدهٔ قبلی» را می‌سازد؛ تستی هست که اگر کسی این را بردارد می‌شکند.
- **گیرندهٔ آزمایشی:** پیش‌فرضِ کانال `log` است و تا وقتی `REBALANCE_ALERT_CHANNELS` صریح تنظیم نشده **هیچ پیامی به هیچ عضوِ واقعی نمی‌رود**. عکسش — «واقعی مگر خاموشش کنی» — یعنی یک اشتباهِ پیکربندی به صندوقِ ورودیِ آدم‌های واقعی می‌رسد و آن خطا برگشت‌پذیر نیست.
- **مسیرِ اجرایی:** `POST /api/portfolio/rebalance-alert`. عمداً POST و نه محاسبه هنگام رندرِ GET؛ وگرنه هر بارگذاری و هر prefetch می‌توانست پیام تولید کند. `user_id` از نشست می‌آید، هرگز از بدنهٔ درخواست.
- **شاهد آزمون (اجرا شد، نه خوانده):** `test:core` **۱۱۵۵** · `test:db` **۳۷۴** روی Postgres 16.13 واقعیِ محلی با `CI=true` و صفر skip · typecheck · lint · build · scan:secrets — همه سبز. کفِ `MIN_PASS` به ۳۷۴ رسید.
- **مسیرِ کامل اثبات شد:** تستی هست که محاسبهٔ واقعی (`buildHoldingsView`) را به تصمیم، به ذخیره در Postgresِ واقعی، و به گیرندهٔ آزمایشی می‌برد و بعد ردِ ماندگارش را در جدول می‌خواند. اجرای دوم پیام دوم نمی‌سازد و عضو دوم مستقل هشدار می‌گیرد.
- **محدودیتی که ادعا نمی‌شود:** ترجمهٔ همین پرس‌وجوها توسط `supabase-js` در `alertStore.ts` آزموده **نشده** — به یک PostgREST زنده نیاز دارد. معناشناسیِ SQL اثبات شده، اتصالِ کلاینت نه. همچنین هیچ تضمینِ مطلقی دربارهٔ ضدتکرارِ خودِ سرویسِ ارسال (Resend/تلگرام) داده نمی‌شود؛ گاردِ این سامانه در لایهٔ رویداد است، نه در لایهٔ تحویلِ آن سرویس‌ها.
- **اجرا نشده:** `phase33` روی Production اجرا نشده و پشتِ همان دروازهٔ بکاپ/بازیابی است. کانالِ واقعی روشن نشده و هیچ پیامی به عضوِ واقعی نرفته.
- **واحدِ ن-۳۰ حالا استخراج می‌شود، نه اعلام.** `unit: "میلیون ریال"` یک ثابتِ سفت بود و اثباتِ استخراج نبود. حالا دو شاهدِ مستقل سنجیده می‌شود: سربرگِ جدول، و سازگاریِ حسابیِ `مبلغ ≈ مقدار × نرخ ÷ مقیاس` روی خودِ همان گزارش. هم‌سو ⇒ واحد با `unit_basis="header+arithmetic"`؛ متعارض ⇒ `unit=null` و `basis="conflict"` بدونِ اینکه یکی برنده شود؛ قالبِ ناشناخته ⇒ `unresolved`. ردیفِ دارای `unit=null` معتبر اعلام نمی‌شود.
- **یک تعمیمِ ممنوع هم بسته شد:** `fy_cumulative_amount: fyCumulative ?? periodTotal` مبلغِ **یک ماه** را به‌عنوان تجمعیِ سال گزارش می‌کرد؛ عددی که در هیچ گزارشی وجود نداشت و برای شرکتی در ماه دوازدهم یک‌دوازدهمِ واقعیت را «تجمعی» نشان می‌داد. حالا نبودِ ستونِ تجمعی `null` می‌ماند.
- دو باگِ خودم حین همین کار پیدا شد و ثبت می‌شود چون قابلِ تکرارند: مرزِ `\b` در regex جاوااسکریپت ASCII-محور است و کنارِ حرفِ فارسی هرگز تطبیق نمی‌کند (`/\bریال\b/` همیشه false بود)، و نمونهٔ آزمونِ اولم یک مرتبهٔ بزرگی خطا داشت که خودِ تستِ سازگاریِ حسابی گرفتش.

### بند ۳ — محیط ورود: مانعِ دقیق، با شاهد

تلاش شد، نه فرض. **تصحیحِ گزارشِ قبلیِ خودم:** قبلاً نوشتم «docker در دسترس
نیست»؛ آن غلط بود — دیمن با `dockerd` بالا می‌آید و `docker info` جواب می‌دهد.
مانع جای دیگری است:

| گام | نتیجه |
|---|---|
| نصب `supabase` CLI ۲.۱۱۷.۰ | ✅ موفق (npm مجاز است) |
| `supabase init` | ✅ موفق |
| راه‌اندازی دیمنِ docker | ✅ موفق |
| `supabase start` (بدونِ studio/realtime/storage) | ❌ **کشیدنِ ایمیج رد شد** |

خطای دقیق از پراکسیِ محیط: `gateway answered 403 to CONNECT` برای
`production.cloudfront.docker.com:443` و `pkg-containers.githubusercontent.com:443`.
سیاستِ شبکه npm و PyPI و crates را مجاز کرده ولی **رجیستریِ کانتینر را نه**.

راهِ دومی هم بسته بود: دانلودِ مستقیمِ باینریِ PostgREST از GitHub، چون دسترسیِ
GitHub این نشست فقط به همین مخزن محدود است (`403` با پیامِ
`GitHub access to this repository is not enabled for this session`).

**پیش‌نیازِ مفقود — یکی از این دو کافی است:**
۱. مجازکردنِ `production.cloudfront.docker.com` و `pkg-containers.githubusercontent.com`
   در سیاستِ شبکهٔ محیط (آن‌وقت `supabase start` کامل محلی اجرا می‌شود و هیچ
   کلید یا دادهٔ واقعی لازم نیست)، یا
۲. کلیدهای یک پروژهٔ Supabase **آزمایشی**: `NEXT_PUBLIC_SUPABASE_URL`،
   `NEXT_PUBLIC_SUPABASE_ANON_KEY`، `SUPABASE_SERVICE_ROLE_KEY` (نام‌ها از
   `lib/supabase/*.ts` و `middleware.ts` استخراج شد، نه حدس) به‌همراه دو حسابِ
   عضوِ آزمایشی.

تا آن زمان، مسیرِ ورود ← ثبت ← ذخیره ← بازکردن ← ویرایش در مرورگر آزموده
نمی‌شود. این کار ساختِ مسیرِ هشدار را متوقف نکرد.

- **نقطه ادامه:** بازبینی مستقل روی این شاخه؛ سپس آزمون مرورگرِ مسیر ورودِ عضو به‌محضِ فراهم‌شدنِ یکی از دو پیش‌نیازِ بالا.

## بازبینی مستقل Claude — ۲۰۲۶-۰۹-۱۷ · تأیید #144 روی `22130ea`

> ⚠️ **درسِ کارِ موازی، دوباره.** این نوبت با یک جلسهٔ هم‌زمان روی همان شاخه
> برخورد کرد؛ هر دو سراغ همان سه ایرادِ #144 رفتیم. پیاده‌سازی این نوبت
> **دور ریخته شد** چون `b2fec9a` فراگیرتر بود (مدل واحد با دو خانوادهٔ
> `tradable`/`mass` و شاهد واقعی مرورگر). §۰ همین را هشدار داده بود و باز
> تکرار شد: **پیش از شروعِ کار روی یک شاخه، `git fetch` و خواندنِ head آن
> اجباری است، نه خواندنِ PR در ابتدای جلسه.**

- تأیید مستقل (اجرا شد، نه خوانده): `test:core` **۱۱۳۷** · `test:db` **۳۶۰** روی Postgres 16.13 واقعیِ محلی با `CI=true` و صفر skip · `typecheck` · `lint:ci` · `build` — همه سبز.
- معماری اصلاح درست است: هر سه یافته ایرادِ **اتصال** بودند نه ایرادِ توابع، و `view.ts` مسیر را یک‌جا می‌کند. `TargetVersion.problems` داخلِ خودِ قرارداد نشسته، پس فراموش‌شدنی نیست.
- دو اصلاح تازهٔ همین نوبت: (۱) دستهٔ ناشناخته در جدول‌های دارایی خام چاپ می‌شد (`commodity` کنارِ «سهام ایران»)؛ `asset_class` در phase32 متنِ آزاد است پس این حالت واقعی است — حالا صریح «(دستهٔ ناشناخته)» علامت می‌خورد بدونِ اختراعِ برچسب. (۲) `node_modules` به‌صورت symlinkِ خودارجاع ردیابی می‌شد و هر checkout تازه را با ELOOP می‌شکست؛ از ایندکس حذف شد.
- دو مورد **طرح‌شده، نه تأییدشده**: (الف) فهرست معطلِ راهنمای بکاپ سه فایل دارد ولی دفتر مهاجرت برای `phase21` و `phase23` هم `NOT_APPLIED` ثبت کرده — یکی از دو سند باید اصلاح شود؛ دسترسی خواندن Production در این نوبت نبود. (ب) خطِ بدونِ فاصلهٔ بالای اسکرین‌شات در کد کامپوننت پیدا نشد و احتمالاً خروجی ابزار اسکرین‌شات است؛ دست‌نخورده ماند چون اصلاحِ بدونِ بازتولید حدس می‌شد.
- تحویل‌نشده‌ها بدون تغییر: migration روی Production اجرا نشده · ارسال هشدار و ذخیرهٔ `AlertState` وصل نیست · آزمون مرورگرِ این نوبت انجام نشد (دسترسی مفقود: کلیدهای Supabase آزمایشی + حساب عضو آزمایشی، یا Supabase محلی که به CLI و دیمنِ docker نیاز دارد — هیچ‌کدام در این کانتینر نیست).
- نقطه ادامه: بازبینی مستقل Codex روی `22130ea`؛ مسیر مستقل بعدی صحت دادهٔ #142.
...
## 0‴. نقطهٔ ادامه — ۱۴۰۵/۰۶/۲۶

پایه: `main@ddaeb97`. شاخهٔ کاری `claude/member-holdings-rebalance` روی `36dd167`.

### چه چیزی در این نوبت بسته شد
- **#144 — سه ایرادِ سبد.** هدفِ نامعتبر دیگر محاسبهٔ قطعی نمی‌دهد (گیت در
  `TargetVersion.problems`) · واحدِ مقدار با واحدِ قیمت سنجیده می‌شود
  (`lib/portfolio/units.ts`) · قیمت با نگاشتِ صریح به `positionKey` می‌رسد
  (`resolvePricesByPosition`) · منبعِ خالی دیگر با «نامشخص» معتبر نمی‌شود.
  هر سه با تزریقِ دوبارهٔ نقص اثبات شدند که تستشان توخالی نیست.
- **اتصال، نه فقط توابع.** `lib/portfolio/view.ts` ترکیبِ واقعیِ صفحه است و
  صفحه خودش دیگر هیچ محاسبه‌ای ندارد.
- **اجرای مرورگر.** حلقهٔ ثبت ← بازکردن ← ویرایش ← مقایسه روی ۱۲۸۰ و ۳۹۰
  پیکسل، ۲۶ بررسیِ سبز، صفر خطای صفحه. تصاویر: `docs/assets/holdings-loop/`.
- **#142 — تفکیکِ پوشش.** «۷۱۲ از ۱۱۴۷» تصحیح شد: ۳۴۳ **شرکت** از ۷۵۶ صورتِ
  مالی ندارند؛ ۳۳۰ صندوق طبیعتاً ن-۱۰ نمی‌دهند. `isin` روی هر ۲٫۱ میلیون
  ردیفِ `symbol_history` خالی است.
- **یک نقصِ دریافتِ داده رفع شد.** ن-۳۰ حالا `unit` و `period_months` ثبت
  می‌کند؛ پیش از این صفر از ۵۷۱۰ ردیف واحد داشت.

### نخستین اقدامِ بعدی
۱. ذخیرهٔ `AlertState` و کانالِ ارسالِ هشدار — امروز **هیچ هشداری به کسی
   نمی‌رسد**؛ موتورِ تصمیم آماده است ولی وصل نیست.
۲. تصمیم دربارهٔ ۵۷۱۰ ردیفِ ن-۳۰ که واحد ندارند (جدول append-only است).
۳. `published_at` در ingestِ کدال — تنها راهِ اثباتِ تازگی.
۴. #142: ذخیره و بازکردنِ دوبارهٔ پروندهٔ تحلیل روی مدلِ موجود.

### مانعِ دیتابیس — دقیق
`docker` و `pg_dump` در محیطِ عامل هستند؛ **رشتهٔ اتصال نیست**. تنها همان
قطعه کم است. ترتیبِ مهاجرت‌ها و وابستگی‌هایشان در
[`RUNBOOK-backup-windows.md`](./RUNBOOK-backup-windows.md) و
[`MIGRATION-LEDGER.md`](./MIGRATION-LEDGER.md) ثبت شد.
