...
|---|---|---|---|---|---|
| **#151** زیرساخت: تریگرِ CI + symlink | `682704c` | `main` | ۷/۷ ✅ | **آمادهٔ ادغام** | فقط تأییدِ ادغام. پیش‌نیازِ #153 و #155 |
...
| **#155** بکاپ: `pgurl.sh` مشترک، رمز بیرون از argv، e2eِ خودِ ps1 (B-057) | `13e7ba5` | #151 | قرمزِ `a647661` رفع شد؛ اجرای تازه در جریان | آمادهٔ بازبینی | PASSِ بکاپ فقط با اجرای واقعی روی لپ‌تاپ |
...
| **#149** هشدار | `09d4893` | #144 | **هرگز اجرا نشده** — فقط ۲ چکِ پیش‌نمایش | **آمادهٔ بازبینی نیست** | اصلاحِ تریگر به‌صورتِ patchِ `7e6fd47` به مالک تحویل شد (sha256 با `40760451871a09ca` شروع می‌شود؛ روی `09d4893` تمیز اعمال می‌شود و `MIN_PASS=377` را نگه می‌دارد). push به این شاخه را محیط رد کرد و به شاخهٔ دیگری فرستاده نشد |
| **#143** لغوِ اعلامیه | `ae06f0a` | `main` | ۷/۷ ✅ (۰۹-۱۷) | آمادهٔ بازبینی | گیتِ #148 روی پایه‌اش هست |
| **#152** دسته‌بندیِ صندوق + سیستمِ بصری | `26a4cf1` | `main` | ۷/۷ ✅ | آمادهٔ بازبینی | **مالکِ فعال دارد** (push امروز ۱۷:۲۸)؛ دست نخورد |
| **#113** پرداخت ← دسترسی | `b64ec25` | `main@2344179` | ۷/۷ روی پایهٔ **کهنه** | **مسدود** | `mergeable_state=dirty`؛ `HOLD_BY_OWNER` (B-025)؛ `D-024` |

**ترتیبِ ادغام:** #151 → (#153، #155) · #150 → #154 · #144 → #149. سه زنجیره مستقل‌اند.
...
- **B-055 — پایانیِ هر جلسه زیرِ برچسبِ جلسهٔ بعد.** از ۱۴۰۵/۰۴/۲۷ هر ۵۰٬۲۱۱ ردیفِ `relay_eod` ساعتِ ۰۰ تهران نوشته شده‌اند. شاهدِ مستقل از کد: سهمِ نمادهای مثبتِ `market_breadth` با `relay_eod`ِ یک برچسب بعد ۵ واحد فاصله دارد، با همان برچسب ۲۱ واحد. اصلاح در #153. **ردیف‌های گذشته اصلاح نمی‌شوند** (append-only)؛ نمای تصحیح در خواندن تصمیمِ مالک است.
  - ⚠️ **هزینهٔ گذار:** کلیدِ یکتای `(symbol, trade_date)` بدونِ `source` است. در روزِ استقرار، برچسبِ آن جلسه از نیمه‌شب با پایانیِ جلسهٔ قبل پر شده، پس پایانیِ **یک جلسه** هرگز ذخیره نمی‌شود. اجتناب‌ناپذیر است؛ **زمانِ دقیقِ استقرار باید ثبت شود** چون هر نمای تصحیحِ بعدی مرزش را از آن می‌گیرد.
...
موجِ سه (پذیرشِ بصری/کنترل‌های #152) انجام نشد — مالکِ فعال دارد و آزمونِ مرورگر به Preview/Production نیاز دارد که از اینجا در دسترس نیست. موجِ پنج (بازبینیِ عمیقِ #143/#144/#149/#113) و موجِ شش (چرخهٔ میزِ تحلیل) شروع نشدند.
