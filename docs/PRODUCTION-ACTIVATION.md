...
### پیش‌نیازِ بکاپ — ⛔ بررسی شد، برقرار **نیست**، و مسیرِ رایگانش آماده است

در `P2-PROD-ACTIVATION-001` این بند بالاخره سنجیده شد و جواب منفی بود:

| بررسی | نتیجه |
|---|---|
| پلنِ سازمانِ Production (`foglnmjnfppaiqdbbgof`) | **`free`** |
| بکاپِ روزانهٔ خودکار | ❌ طبقِ مستنداتِ Supabase فقط `Pro`/`Team`/`Enterprise` |
| PITR | ❌ افزونهٔ پلن‌های پولی |
| بکاپِ دستی از محیطِ Claude | ❌ هاستِ دیتابیس از آنجا در دسترس نیست (`B-030`) و رمز هم در اختیارِ Claude نیست و **نباید باشد** |
...
> their data** using the Supabase CLI `db dump` command.

**تصمیمِ مالک (۱۴۰۵/۰۵/۱۳): ارتقای پولی انجام نمی‌شود؛ مسیرِ رایگان اجرا شود.**

### مسیرِ رایگان — یک دستور، روی کامپیوترِ آرش

```bash
bash scripts/backup-production.sh
```

پیش‌نیاز: Docker (در حالِ اجرا) و Supabase CLI — یا فقط `npx`.

اسکریپت طبقِ روشِ رسمیِ Supabase سه فایلِ جدا می‌سازد (`roles` · `schema` ·
`data`)، و سه قاعده را رعایت می‌کند:

- **رشتهٔ اتصال پرسیده می‌شود، ذخیره نمی‌شود.** با `read -rs` گرفته می‌شود؛ نه
  روی دیسک می‌رود، نه در تاریخچهٔ شل، نه در خروجی. **در چت برای کسی فرستاده
  نمی‌شود.**
- **فایل‌ها بیرونِ مخزن ساخته می‌شوند** (`~/supabase-backups/prod-<timestamp>`)
  و اگر مقصد داخلِ یک ریپوی گیت باشد اسکریپت **اجرا نمی‌شود**. `.gitignore` هم
  لایهٔ دوم است.
- **بکاپ فقط وقتی «سالم» اعلام می‌شود که واقعاً بازگردانده شده باشد.** اسکریپت
  یک Postgresِ یک‌بارمصرف در Docker بالا می‌آورد، هر سه فایل را در آن restore
  می‌کند و شمارشِ ردیف‌ها را با Production مقایسه می‌کند. اگر یکی نخواند،
  با خطا تمام می‌شود.

نکتهٔ طراحی: شمارشِ مرجع **از خودِ دیتابیس در همان لحظه** گرفته می‌شود، نه از
عددی که در فایل نوشته باشیم — وگرنه با رشدِ داده بی‌صدا کهنه می‌شد و
راستی‌آزمایی به یک تیکِ بی‌معنا تبدیل می‌شد.

خروجی‌اش یک `MANIFEST.txt` است با تاریخ، اندازه و `sha256`ِ هر سه فایل. **فقط
همان چند خط** برای Claude فرستاده می‌شود — هیچ‌کدام حساس نیستند.

⚠️ تنها نشتِ باقی‌مانده: `supabase db dump` رشتهٔ اتصال را به‌صورتِ آرگومان
می‌گیرد، پس تا لحظهٔ اجرا در `ps` دیدنی است. روی لپ‌تاپِ شخصی اهمیتِ عملی ندارد؛
روی ماشینِ مشترک اجرا نشود.

### بازگشتِ همین سه migration

مستقل از بکاپ، هر سه migrationِ مرحلهٔ ۱ **دادهٔ موجود را لمس نمی‌کنند**:

| migration | تغییر | بازگشت |
|---|---|---|
| `phase8b` | فقط `CREATE TABLE leads` | `DROP TABLE public.leads` |
| `phase21` | فقط `CREATE TABLE cron_runs` | `DROP TABLE public.cron_runs` |
| `phase23` | فقط `REVOKE` | وضعیتِ پیش از تغییر در جدولِ زیر ثبت شده |

**وضعیتِ گرنت‌های نوشتن پیش از `phase23`** (اندازه‌گیریِ فقط‌خواندنیِ
۱۴۰۵/۰۵/۱۳ — مجموعاً ۷۳۲ گرنت، به‌علاوهٔ ۱۲۲ گرنتِ `SELECT` که `phase23` اصلاً
لمسشان نمی‌کند و مجموع ۸۵۴ را می‌سازد):

| نقش | امتیازها | تعدادِ جدول |
|---|---|---|
| `anon` | `DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE` | ۴۰ (همه به‌جز `fx_heavy_analytics`) |
| `authenticated` | همان شش | ۴۱ (همه) |
| `service_role` | همان شش | ۴۱ (همه) |

> این جدول عمداً **توضیح** است و نه اسکریپتِ اجرایی. نسخهٔ اولِ این سند یک فایلِ
> `.sql` با ۷۳۲ دستورِ `GRANT` داشت؛ چون این مخزن **عمومی** است، آن فایل عملاً
> یک اسکریپتِ آمادهٔ «بازکردنِ دوبارهٔ `B-044`» بود. حذف شد. اگر روزی بازگردانی
> لازم شود، از روی همین سه سطر ساختنی است و آن لحظه باید تصمیمِ آگاهانه باشد،
> نه یک `psql -f`.

⚠️ هیچ‌کدامِ این‌ها **جایگزینِ بکاپ نیست**؛ فقط همین سه تغییر را برمی‌گرداند.
