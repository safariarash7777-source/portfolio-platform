...
> **درفتِ کامنتِ کد ↔ `vercel.json` — ✅ رفع شد (2026-07-29، `G2-008`/`G2-007`):**
> کامنتِ `app/api/cron/alerts/route.ts` قبلاً «هر ۵ دقیقه (Vercel Cron)» می‌گفت در حالی
> که `vercel.json` این مسیر را **روزانه `0 6 * * *`** زمان‌بندی کرده بود. همان ادعا در
> `components/landing/ProductFacts.tsx` و **`components/landing/Capabilities.tsx`** هم بود
> (دومی در ممیزیِ اول دیده نشده بود). هر سه به «روزانه» اصلاح شدند و
> `lib/core/cadence.test.ts` حالا برگشتِ این درفت را در CI مردود می‌کند.
>
> **`vercel.json` منبعِ حقیقتِ زمان‌بندی است.**
>
> چهار چیزِ متفاوت که مدام با هم اشتباه می‌شوند و باید جدا بمانند:
>
> | مفهوم | کجاست | تضمین دارد؟ |
> |---|---|---|
> | **عمرِ کش** | `CACHE_MS = 5 دقیقه` در `lib/market.ts:42` | — فقط بازاستفادهٔ داده |
> | **تازه‌سازیِ ترافیک‌محور** | `lib/market.ts:59` | ❌ فقط اگر درخواستی برسد |
> | **Cronِ زمان‌بندی‌شده** | `vercel.json` | ✅ تنها مسیرِ تضمین‌شده |
> | **آخرین اجرای موفقِ واقعی** | `/admin/health` | مشاهده‌ای، نه ادعایی |
>
> ⚠️ **پیامدِ محصولی که همچنان برقرار است:** هشدارهای قیمت **روزی یک‌بار** ارزیابی
> می‌شوند. زمان‌بندی عمداً دست نخورد — متراکم‌کردنش احتمالاً ارتقای پلنِ Vercel
> می‌خواهد و تصمیمِ جداست. متن به واقعیت آمد، نه برعکس.
>
> ⚠️ **اجرای واقعی مشاهده نشده:** زمان‌بندی **پیکربندی** شده است؛ هیچ اجرای موفقِ
> تاریخ‌داری در این سشن دیده نشد (نگهداریِ لاگِ Vercel کوتاه‌تر از آن است). پس
> «زمان‌بندی‌شده» ✅ ولی «عملیاتی» **UNVERIFIED**. `/admin/health` پس از استقرار
> جواب می‌دهد.
